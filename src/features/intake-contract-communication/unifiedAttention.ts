...
const one=<T extends readonly string[]>(v:unknown,a:T)=>{if(typeof v!=='string'||!(a as readonly string[]).includes(v))bad();return v as T[number]};
...
const bool=(v:unknown)=>{if(typeof v!=='boolean')bad();return v};
function item(v:unknown):UnifiedAttentionItem{const r=rec(v),kind=one(r.kind,K),decision=r.decision===null?null:one(r.decision,['approved','rejected'] as const);return Object.freeze({id:id(r.id),kind,authority:one(r.authority,['intake_submissions','client_portal_requests','renewals'] as const),evidenceAuthority:one(r.evidenceAuthority,['intake_followup_requests','contract_approval_bridge_bindings','contract_renewal_communication_evidence'] as const),canonicalId:id(r.canonicalId),title:str(r.title),state:one(r.state,S),attentionState:one(r.attentionState,A),stale:bool(r.stale),mode:one(r.mode,['secure_link','client_portal','canonical_renewal'] as const),requestKind:one(r.requestKind,['information','document','approval','renewal'] as const),dueAt:time(r.dueAt),decision,communicationEvidence:bool(r.communicationEvidence),companyId:optId(r.companyId),transactionId:optId(r.transactionId),contractRevisionId:optId(r.contractRevisionId),sourceVersion:ver(r.sourceVersion),canonicalVersion:ver(r.canonicalVersion),ownerSurface:one(r.ownerSurface,O)})}
function view(v:unknown):UnifiedAttentionView{const r=rec(v),generatedAt=time(r.generatedAt);if(r.schema!=='enjaz.intake-contract-attention.v1'||typeof r.includeTerminal!=='boolean'||!Array.isArray(r.items)||generatedAt===null)bad();return Object.freeze({schema:'enjaz.intake-contract-attention.v1',workspaceId:id(r.workspaceId),workspaceTimezone:str(r.workspaceTimezone,120),kind:one(r.kind,F),includeTerminal:r.includeTerminal,generatedAt,items:Object.freeze(r.items.map(item))})}
