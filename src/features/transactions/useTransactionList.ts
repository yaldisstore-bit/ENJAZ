...
export function useTransactionList():TransactionListController{
  ...
  useEffect(()=>{const t=token.current;publishTransactionSavedViewBridge(t,request,(next:Partial<TransactionListRequest>)=>setRequest(normalizeTransactionListRequest({...next,page:0})));return()=>clearTransactionSavedViewBridge(t)},[request]);
  const snapshot=useMemo(()=>source?buildTransactionListSnapshot(source,request):null,[request,source]);
  return Object.freeze({status,snapshot,request,errorMessage,retry(){setAttempt(x=>x+1)},setView(view:TransactionListView){setRequest(x=>normalizeTransactionListRequest({...x,view,page:0}))},setSearch(search:string){setRequest(x=>normalizeTransactionListRequest({...x,search,page:0}))},setSort(sort:TransactionListSort){setRequest(x=>normalizeTransactionListRequest({...x,sort,page:0}))},setPage(page:number){setRequest(x=>normalizeTransactionListRequest({...x,page}))},applyRequest(next:Partial<TransactionListRequest>){setRequest(normalizeTransactionListRequest({...next,page:0}))}});
}
