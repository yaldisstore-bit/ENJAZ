...
export interface GovernanceStake { readonly id:string; readonly holderKind:HolderKind; readonly holderId:string; readonly role:OwnershipRole; readonly percentage:string; readonly effectiveFrom:string; readonly effectiveTo:string|null }
export interface BeneficialOwnerView { readonly id:string; readonly contactId:string; readonly displayName:string; readonly basis:BeneficialBasis; readonly percentage:string|null; readonly effectiveFrom:string; readonly effectiveTo:string|null }
export interface AuthorityView { readonly id:string; readonly contactId:string; readonly displayName:string; readonly role:GovernanceRole; readonly scope:AuthorityScope; readonly powers:readonly string[]; readonly effectiveFrom:string; readonly effectiveTo:string|null; readonly endReason:string|null }
export interface ResolutionView { readonly id:string; readonly number:string|null; readonly title:string; readonly type:ResolutionType; readonly effectiveOn:string; readonly notes:string|null }
export interface GovernanceTimelineEvent { readonly id:string; readonly type:string; readonly effectiveOn:string; readonly version:number; readonly occurredAt:string; readonly details:Readonly<Record<string,unknown>> }
export interface GovernanceRisk { readonly code:string; readonly severity:'high'|'medium'|'low'; readonly message:string }
export interface GovernanceContext {
  readonly companyId:string; readonly asOf:string; readonly canMutate:boolean;
  ...
  readonly beneficialOwners:readonly BeneficialOwnerView[]; readonly authorities:readonly AuthorityView[]; readonly resolutions:readonly ResolutionView[];
  readonly capital:Readonly<{known:boolean;amount:string|null;source:string;effectiveOn:string|null;version:number}>;
  readonly timeline:readonly GovernanceTimelineEvent[]; readonly risks:readonly GovernanceRisk[];
}
...
const UUID=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i,DATE=/^\d{4}-\d{2}-\d{2}$/,OWNERSHIP=/^(?:0|[1-9]\d{0,2})(?:\.\d{1,6})?$/,MONEY=/^(?:0|[1-9]\d{0,15})(?:\.\d{1,2})?$/,TIMEOUT=15_000;
...
function parseContext(value:unknown):GovernanceContext{
  const r=rec(value);if(r.schema!=='enjaz.governance-context.v1')throw new DataAccessError('Unsupported governance schema','DATA_OPERATION_FAILED');const versions=rec(r.versions),ownership=rec(r.ownership),capital=rec(r.capital);
...
export function createGovernanceCommandGateway(client:EnjazSupabaseClient,timeoutMs=TIMEOUT):GovernanceCommandGateway{
  if(!Number.isSafeInteger(timeoutMs)||timeoutMs<1||timeoutMs>120_000)throw new Error('Invalid governance timeout');const c=client as unknown as RpcClientLike;
  ...
  const common=(workspaceId:string,companyId:string)=>{inputId(workspaceId,'workspace');inputId(companyId,'company')},op=(operationId:string)=>inputId(operationId,'operation');
  const gateway:GovernanceCommandGateway={
