...
import type { RiskEvidence, RiskSeverity, RiskSignal, RiskUrgency } from './riskEngine.ts';

const PAGE=100;
export const RISK_SOURCE_LIMIT=10_000;
export class RiskWorkspaceUnavailableError extends Error{constructor(){super('Smart Risk workspace unavailable');this.name='RiskWorkspaceUnavailableError'}}
export class RiskSourceCapacityError extends Error{readonly sourceName:string;constructor(sourceName:string){super(`Risk source limit: ${sourceName}`);this.name='RiskSourceCapacityError';this.sourceName=sourceName}}
export class RiskSourcePageStalledError extends Error{readonly sourceName:string;constructor(sourceName:string){super(`Risk source stalled: ${sourceName}`);this.name='RiskSourcePageStalledError';this.sourceName=sourceName}}
...
const ev=(sourceDomain:RiskEvidence['sourceDomain'],sourceObjectId:string,field:string,observedValue:RiskEvidence['observedValue'],observedAt:string|null):RiskEvidence=>({sourceDomain,sourceObjectId,field,observedValue,observedAt});

export async function loadSmartRisk(d:SmartRiskDependencies,userId:string,now:Date=new Date()):Promise<SmartRiskLiveResult>{
 if(!Number.isFinite(now.getTime()))throw new Error('Invalid risk evaluation time');
 ...
 if(f.workspaceId!==workspaceId)throw new RiskAuthorityDriftError('Finance workspace drift');fieldOk(field);
 ...
 for(const x of active){
  ...
  if(x.status==='stalled'){const critical=x.priority==='urgent';push('transaction_stalled',critical?'critical':'high','now',entity,{code:'status_stalled',explanation:'المعاملة متوقفة.'},[ev('transactions',x.id,'status',x.status,evaluatedAt)],critical?'معاملة عاجلة متوقفة.':'معاملة متوقفة تحتاج مراجعة.','/app/transactions')}
  const last=x.last_activity_at?Date.parse(x.last_activity_at):NaN,h=(nowMs-last)/3_600_000;if(Number.isFinite(last)&&last<=nowMs&&h>=48)push('transaction_inactive',h>=96?'high':'medium',h>=96?'now':'soon',entity,{code:'inactivity_window',explanation:`لا حركة منذ ${Math.floor(h)} ساعة.`},[ev('transactions',x.id,'last_activity_at',x.last_activity_at!,evaluatedAt)],`آخر حركة أقدم من 48 ساعة.`,'/app/transactions');
 }
 const openBlockers=blockers.filter(x=>x.status==='open');
 for(const b of openBlockers){if(b.severity!=='high'&&b.severity!=='critical')continue;const x=activeById.get(b.transaction_id);if(!x)continue;const label=x.legacy_id?.trim()?`معاملة ${x.legacy_id!.trim()}`:x.type.trim()||undefined,entity=label?{type:'transaction' as const,id:x.id,label}:{type:'transaction' as const,id:x.id};push('open_critical_blocker',b.severity,'now',entity,{code:`blocker_${b.severity}`,explanation:'حاجز مفتوح عالي الأثر.'},[ev('transactions',b.id,'status',b.status,b.opened_at),ev('transactions',b.id,'severity',b.severity,b.opened_at)],'حاجز عالي/حرج مفتوح.','/app/transactions')}
 const financeSignals=f.snapshot.signals.filter(x=>x.severity!=='info');
 for(const x of financeSignals){const severity:RiskSeverity=x.severity==='high'?'high':'medium';push('finance_anomaly',severity,severity==='high'?'now':'soon',{type:'finance',id:x.id,label:x.title},{code:`finance_7_3:${x.id}`,explanation:x.explanation},[ev('finance',x.id,'anomaly_kind',`finance_7_3:${x.id}`,f.snapshot.asOf)],x.explanation,'/app/finance')}
