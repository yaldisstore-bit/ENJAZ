...
function parseItem(value:unknown):UnifiedCalendarItem{const r=record(value),rawStaff=r.staffMemberIds;if(!Array.isArray(rawStaff))fail();const staff=rawStaff as unknown[];id(r.id);one(r.source,SOURCES);text(r.authority,80);text(r.title,320);maybeInstant(r.startsAt);maybeInstant(r.endsAt);maybeDay(r.dueDate);text(r.status,80);one(r.attentionState,ATTENTION);maybeId(r.transactionId);maybeId(r.companyId);maybeText(r.companyLabel,320);staff.forEach(id);maybeText(r.confirmationStatus,80);maybeText(r.attendanceOutcome,80);Object.freeze(staff);return Object.freeze(r) as unknown as UnifiedCalendarItem}

export function parseUnifiedCalendarSnapshot(value:unknown):UnifiedCalendarSnapshot{const r=record(value),rawItems=r.items;if(r.schema!=='enjaz.unified-calendar.v2'||!Array.isArray(rawItems))fail();const items=rawItems as unknown[];text(r.workspaceTimezone,120);one(r.view,VIEWS);day(r.anchorDate);day(r.businessToday);day(r.businessStartDate);day(r.businessEndDate);instant(r.generatedAt);return Object.freeze({...r,items:Object.freeze(items.map(parseItem))}) as unknown as UnifiedCalendarSnapshot}

export async function loadUnifiedCalendar(factory:EnjazDataLayerFactory,input:UnifiedCalendarQuery):Promise<UnifiedCalendarSnapshot>{const rpc=factory.rpc;if(!rpc)throw Error('Unified calendar RPC unavailable');const result=await rpc('list_unified_calendar_v2',{p_workspace_id:input.workspaceId,p_anchor_date:input.anchorDate??null,p_view:input.view,p_authority:input.authority??'all',p_staff_member_id:input.staffMemberId??null,p_company_id:input.companyId??null,p_transaction_id:input.transactionId??null,p_limit:input.limit??500});if(result.error)fail();return parseUnifiedCalendarSnapshot(result.data)}
