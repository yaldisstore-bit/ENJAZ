...
function CompanyEditor(props: Readonly<{ mode: 'create' | 'edit'; source: CompanyDetailSource | null; onClose: () => void; onSaved: (id: string) => void }>) {
  ...
  return <section className="r2-records-entity-profile r2-company-editor r2-core-editor" data-company-editor={props.mode}>
    ...
    <form className="r2-golden-form r2-core-form" noValidate onSubmit={(event) => { event.preventDefault(); void controller.save(); }}><div className="r2-golden-form__grid">
      ...
      <label><span>رأس المال</span><input inputMode="decimal" value={controller.draft.capitalInput} onChange={field('capitalInput')} placeholder="مثال: 100000000" />{errorFor('capitalInput')}</label><label><span>الوضع القانوني</span><input value={controller.draft.legalStatus} onChange={field('legalStatus')} />{errorFor('legalStatus')}</label>
...
export function ConnectedCompanies({ openTransaction }: Readonly<{ openTransaction?: ((id: string) => void) | undefined }> = {}) {
  ...
  else main = <div className="r2-company-detail" data-company-profile={detail.source.company.id}><div className="r2-golden-form__actions"><button type="button" className="r2-action r2-action--secondary" disabled={detail.source.company.merged_into_id !== null} onClick={() => setEditorMode('edit')}>تعديل البيانات</button></div><Entity360Panel source={buildCompany360Source(detail.source)} openTransaction={openTransaction} /></div>;
  ...
  return <section className="r2-screen r2-records-screen r2-companies-connected" data-records-domain="companies" data-phase6-1="companies" data-company-source="workspace">
    ...
    <aside className="r2-records-truth" role="note"><strong>حدود المجالات محفوظة</strong><span>إدارة الأشخاص والعلاقات الكاملة تبقى Phase 6.2 كمصدرها القانوني، وCompany/Lawyer 360° تبقى Phase 6.3 كسطح القراءة الموحد، والمالية الكاملة Phase 7 والوثائق Phase 10.</span></aside>
