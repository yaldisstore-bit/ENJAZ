...
type Props = Readonly<{
  ...
  searchWorkspace: Promise<string | null>;
  searchUserId: string;
}>;
...
function SearchOverlay({ query, setQuery, close, navigate, openTransaction, gateway, workspace }: { query: string; setQuery: (value: string) => void; close: () => void; navigate: (id: R2DestinationId) => void; openTransaction: (id: string) => void; gateway: SearchIntelligenceGateway; workspace: Promise<string | null> }) {
  const results = useMemo(() => buildR2FindAnythingResults(query), [query]);
  const remote = useGlobalSearch(query, gateway, workspace);
  const openAuthoritativeResult = (item: GlobalSearchResultReference) => {
    const url = new URL(window.location.href);
    url.searchParams.delete('entity');
    url.searchParams.delete('procedure');
    if (item.domain !== 'transactions') url.searchParams.set(item.domain === 'procedures' ? 'procedure' : 'entity', item.entityId);
    window.history.replaceState(window.history.state, '', url);
    if (item.domain === 'transactions') openTransaction(item.entityId);
    else navigate((item.domain === 'procedures' ? 'workflow' : item.domain) as R2DestinationId);
  };
  return <div className="r2-overlay" role="dialog" aria-modal="true" data-overlay="search" data-zero-lost-search="R2.0-8" data-phase9-2-global-search="authoritative" aria-labelledby="r2-search-title"><button type="button" className="r2-overlay__backdrop" aria-label="إغلاق البحث" onClick={close} /><section className="r2-search-panel"><div className="r2-search-panel__head"><div><p className="r2-eyebrow">Find Anything · 9.2</p><h2 id="r2-search-title">ابحث عن أي شيء</h2></div><button type="button" className="r2-close-button" onClick={close} aria-label="إغلاق">×</button></div><label className="r2-search-input-wrap"><Icon name="search" /><input autoFocus value={query} maxLength={120} onChange={(event) => setQuery(event.target.value)} placeholder="معاملة، شركة، شخص، إجراء أو وثيقة…" /></label><div className="r2-search-results" aria-live="polite">
    {remote.status === 'loading' ? <p className="r2-search-authority-state">جارٍ البحث…</p> : null}
    {remote.status === 'error' ? <p className="r2-search-authority-state is-error">{remote.errorMessage}</p> : null}
    {remote.status === 'ready' ? SEARCH_DOMAINS.map(([domain, label]) => {
      const rows = remote.results.filter((item) => item.domain === domain);
      return rows.length ? <section className="r2-search-live-group" key={domain} data-global-search-domain={domain}><div className="r2-search-live-group__title"><strong>{label}</strong><span>{rows.length}</span></div>{rows.map((item) => <button type="button" key={`${item.domain}:${item.entityId}`} className="r2-search-result" data-global-search-destination={item.destination} onClick={() => openAuthoritativeResult(item)}><span className="r2-search-result__icon"><Icon name={item.domain === 'transactions' ? 'transactions' : 'module'} /></span><span><strong>{item.title}</strong><small>{item.subtitle}</small></span><Icon name="arrow" /></button>)}</section> : null;
    }) : null}
    {query.trim().length >= 2 && remote.status === 'ready' && !remote.results.length ? <p className="r2-search-authority-state">لا توجد سجلات مطابقة.</p> : null}
    {results.length ? <><div className="r2-search-local-divider">اختصارات إنجاز</div>{results.map((item) => <button type="button" key={item.key} className="r2-search-result" data-find-kind={item.kind} data-find-source={item.source} onClick={() => item.kind === 'transaction' && item.transactionId ? openTransaction(item.transactionId) : navigate(item.destinationId)}><span className="r2-search-result__icon"><Icon name={item.kind === 'transaction' ? 'transactions' : 'module'} /></span><span><strong>{item.label}</strong><small>{item.secondary}</small></span><Icon name="arrow" /></button>)}</> : null}
    {!results.length && remote.status !== 'loading' && !remote.results.length ? <p className="r2-search-empty">لا توجد نتيجة مطابقة. لا يختلق إنجاز سجلات غير موجودة.</p> : null}
  </div><p className="r2-search-footnote" data-alias-count={SEARCH_ALIAS_COUNT}>البحث يحافظ على عقد Zero-Lost الحالي؛ ربط سجلات الإنتاج الأوسع يمر عبر Data Layer فقط.</p></section></div>;
}
...
export function UiR2LiveRoot({ accountLabel = 'حساب إنجاز', onSignOut, searchIntelligence, searchWorkspace, searchUserId }: Props) {
  ...
  return <div className="ez-r2-root r2-shell" data-r2-shell="R2.0-3" data-r2-runtime-mode="live" data-golden-stage="R2.0-4" data-core-work-stage="R2.0-5" data-records-stage="R2.0-6" data-operational-stage="R2.0-7" data-zero-lost-stage="R2.0-8" data-phase9-2-runtime="search-saved-views" data-destination={destinationId}>
    ...
    {overlay === 'search' && <SearchOverlay query={searchQuery} setQuery={setSearchQuery} close={closeOverlay} navigate={navigate} openTransaction={openTransaction} gateway={searchIntelligence} workspace={searchWorkspace} />}
