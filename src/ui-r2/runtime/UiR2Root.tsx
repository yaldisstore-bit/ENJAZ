...
function Transactions({ navigate }: { navigate: (id: R2DestinationId) => void }) {
  ...
  return (
    <div className="r2-screen" data-screen="transactions">
      <div className="r2-section-heading r2-section-heading--hero"><div><p className="r2-eyebrow">Golden Experience · المجال التشغيلي الأساسي</p><h1>المعاملات</h1><p className="r2-supporting">قائمة واضحة، بحث داخل السياق، وهوية قوية لكل سجل. الرحلة الذهبية الكاملة مثبتة على المعاملة 1042.</p></div><ActionButton className="r2-action r2-action--primary" onClick={() => navigate('create')}><Icon name="plus" /> معاملة جديدة</ActionButton></div>
      <div className="r2-transaction-tools">
        ...
        <div className="r2-segment" aria-label="تقسيم المعاملات">{(['جارية', 'متلكئة', 'مغلقة'] as const).map((item) => <button key={item} type="button" aria-pressed={segment === item} onClick={() => setSegment(item)}>{item}</button>)}</div>
      </div>
      <section className="r2-record-list" aria-label="عينة Golden للمعاملات">
        {visibleRows.length ? visibleRows.map((row, index) => row.goldenJourney ? (
          <button type="button" className="r2-record-row r2-record-row--golden" key={row.id} onClick={() => navigate('transactions.detail')} aria-label={`فتح المعاملة ${row.id} ${row.title}`}><span className="r2-record-row__number">#{row.id}</span><span className="r2-record-row__identity"><strong>{row.title}</strong><small>{row.company}</small></span><span className="r2-record-row__meta">{row.meta}</span><span className="r2-record-row__index">Golden</span><Icon name="arrow" /></button>
        ) : (
          <article className="r2-record-row r2-record-row--static" key={row.id}><span className="r2-record-row__number">#{row.id}</span><span className="r2-record-row__identity"><strong>{row.title}</strong><small>{row.company}</small></span><span className="r2-record-row__meta">{row.meta}</span><span className="r2-record-row__index">0{index + 1}</span><span className="r2-record-row__sample">عينة قائمة</span></article>
...
function Today({ navigate }: { navigate: (id: R2DestinationId) => void }) {
  const items = [['09:30', 'متابعة معاملة 1038', 'المعاملات', 'transactions'], ['12:00', 'مراجعة وثيقة واردة', 'الوثائق', 'documents'], ['14:30', 'قرار مالي يحتاج مراجعة', 'المالية', 'finance']];
  return <div className="r2-screen" data-screen="today"><div className="r2-section-heading r2-section-heading--hero"><div><p className="r2-eyebrow">الآن</p><h1>اليوم</h1><p className="r2-supporting">العمل الذي يحتاجك اليوم بترتيب واضح، بدل تشتيت المهام في أنحاء التطبيق.</p></div></div><section className="r2-timeline">{items.map(([time, title, context, id]) => <button type="button" key={time} className="r2-timeline-row" onClick={() => navigate(id as R2DestinationId)}><time>{time}</time><span className="r2-timeline-row__track"><i /></span><span className="r2-timeline-row__copy"><strong>{title}</strong><small>{context}</small></span><Icon name="arrow" /></button>)}</section></div>;
}

function Create({ navigate }: { navigate: (id: R2DestinationId) => void }) {
  return <div className="r2-screen" data-screen="create"><div className="r2-section-heading r2-section-heading--hero"><div><p className="r2-eyebrow">إجراء عالمي</p><h1>ماذا تريد أن تنشئ؟</h1><p className="r2-supporting">نقطة إنشاء واحدة. الخيارات لا تظهر كوظائف حقيقية قبل ربطها ببياناتها وصلاحياتها.</p></div></div><button type="button" className="r2-create-primary" onClick={() => navigate('transactions.editor')}><span className="r2-create-primary__icon"><Icon name="transactions" /></span><span><strong>معاملة جديدة</strong><small>يفتح محرر Golden التفاعلي دون كتابة في بيانات الإنتاج</small></span><Icon name="arrow" /></button><div className="r2-create-later"><span>خيارات أخرى</span><p>ستظهر هنا فقط عندما تصبح مدعومة ببيانات حقيقية وعقد صلاحيات واضح.</p></div></div>;
}

function More({ navigate }: { navigate: (id: R2DestinationId) => void }) {
  return <div className="r2-screen" data-screen="more"><div className="r2-section-heading r2-section-heading--hero"><div><p className="r2-eyebrow">خريطة النظام · Golden</p><h1>المزيد</h1><p className="r2-supporting">كل قدرة لها منزل واحد واضح. المجموعات منفصلة بصريًا لكن تبقى صفحة واحدة متماسكة، دون جدار بطاقات.</p></div></div><div className="r2-launcher-groups">{R2_LAUNCHER_GROUPS.map((group) => <section className="r2-launcher-group" key={group.id}><h2>{group.label}</h2><div className="r2-launcher-list">{group.destinations.map((id) => <button type="button" key={id} className="r2-launcher-row" onClick={() => navigate(id)}><span className="r2-launcher-row__icon"><Icon name="module" /></span><span className="r2-launcher-row__copy"><strong>{getR2Destination(id).label}</strong><small>منزل قانوني واحد</small></span><Icon name="arrow" /></button>)}</div></section>)}</div></div>;
}
...
function Destination({ id, navigate }: { id: R2DestinationId; navigate: (id: R2DestinationId) => void }) {
  ...
  if (id.startsWith('transactions.')) return <GoldenTransactionExperience id={id as Extract<R2DestinationId, `transactions.${string}`>} navigate={navigate} />;
  return <div className="r2-screen r2-destination-placeholder" data-screen="launcher-destination"><div className="r2-destination-mark"><Icon name="module" /></div><p className="r2-eyebrow">وجهة مثبتة في بنية إنجاز الجديدة</p><h1>{destination.label}</h1><p>الـShell يعرف مكان هذه الميزة وطريق الوصول إليها. محتوى المجال نفسه سيُبنى في مرحلة الترحيل المخصصة له؛ R2.0-4 يصقل طبقات الهوية والعنوان والوصف والأفعال دون ادعاء بيانات غير موجودة.</p><div className="r2-placeholder-actions"><ActionButton className="r2-action r2-action--primary" onClick={() => navigate('more')}>العودة إلى المزيد</ActionButton><ActionButton className="r2-action r2-action--secondary" onClick={() => navigate('home')}>الرئيسية</ActionButton></div></div>;
}
...
export function UiR2Root() {
  ...
  useEffect(() => {
    const syncFromHistory = () => { const next = readUrlState(); setDestinationId(next.destinationId); setOverlay(next.overlay); ownedOverlay.current = false; };
  ...
  const closeOverlay = () => {
    ...
    else { writeUrlState(destinationId, null, 'replace'); setOverlay(null); }
  };
  ...
  const navigate = (id: R2DestinationId) => { writeUrlState(id, null); setDestinationId(id); setOverlay(null); ownedOverlay.current = false; window.scrollTo({ top: 0, behavior: 'auto' }); };
  const openOverlay = (id: Exclude<OverlayId, null>) => { writeUrlState(destinationId, id); setOverlay(id); ownedOverlay.current = true; };
