...
function createRuntimeRevision(input:CreateEngagementContractRevisionInput):EngagementContractRuntimeRevision{
  ...
  return Object.freeze({workspaceId:W,engagementId:E,revision,title:input.title,status:'draft',templateVersionId:TEMPLATE,draftId:DRAFT,documentId:null,documentVersionId:null,supersedesRevision:previous?.revision??null,effectiveOn:null,expiresOn:null,signedAt:null,id,signatureProvenance:Object.freeze({}),terminationNote:null,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
}
...
const contractGateway:EngagementContractGateway={
  ...
  async transition(input:TransitionEngagementContractRevisionInput){await pause(15);if(input.workspaceId!==W)throw new Error('WORKSPACE_FORBIDDEN');const current=revisions.find(r=>r.id===input.revisionId);if(!current)throw new Error('CONTRACT_NOT_FOUND');assertEngagementContractTransition(current.status,input.toStatus);let documentId=current.documentId,documentVersionId=current.documentVersionId,signedAt=current.signedAt,effectiveOn=current.effectiveOn,expiresOn=current.expiresOn,signatureProvenance=current.signatureProvenance,terminationNote=current.terminationNote;
