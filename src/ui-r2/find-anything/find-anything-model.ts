...
export function buildR2FindAnythingResults(rawQuery:string,options:{limit?:number;records?:readonly SearchableRecord[]}={}):readonly R2FindAnythingResult[]{
  ...
  if(!query)return SHORTCUTS.slice(0,limit).map((id,index)=>({key:`feature:${id}`,kind:'feature',label:getR2Destination(id).label,secondary:'اختصار سريع · منزل قانوني واحد',destinationId:id,transactionId:null,source:'navigation',score:100-index}));
  const records=options.records??(globalThis as SearchGlobal).__ENJAZ_R2_PREVIEW_SEARCH_RECORDS__??[];
  const recordResults:R2FindAnythingResult[]=records.map(record=>({key:record.key,kind:'transaction',label:record.label,secondary:record.secondary,destinationId:'transactions.detail',transactionId:record.transactionId,source: 'preview-record',score:score(query,record.terms)})).filter(x=>x.score>0);
  const featureResults:R2FindAnythingResult[]=R2_DESTINATIONS.filter(x=>x.kind!=='system_boundary').map(item=>({key:`feature:${item.id}`,kind:'feature',label:item.label,secondary:item.kind==='launcher_destination'?'ميزة · منزل قانوني واحد':'وجهة نظام',destinationId:item.id,transactionId:null,source:'navigation',score:score(query,[item.label,item.id,...Object.entries(R2_SEARCH_ALIASES).filter(([,id])=>id===item.id).map(([alias])=>alias)])})).filter(x=>x.score>0);
