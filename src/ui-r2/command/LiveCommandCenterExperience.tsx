  ...
  return <div className="r2-screen r2-command-live" data-command-stage="8.6" data-command-authority="delegated_existing_domain_gateways_only" data-command-write-authority="none" data-finance-write-authority="none">
    ...
    {!snapshot?<section className="r2-command-state"><strong>{message===FAIL?'صورة القيادة غير متاحة':'جارٍ تكوين صورة القيادة…'}</strong>{message===FAIL&&<button type="button" className="r2-action r2-action--secondary" onClick={()=>void load().catch(()=>setMessage(FAIL))}>إعادة المحاولة</button>}</section>:<>
      ...
      <section className="r2-command-grid r2-command-grid--lower">
        <article className="r2-command-panel"><div className="r2-command-panel__head"><div><p className="r2-eyebrow">Operational Ownership</p><h2>إعادة إسناد العمل الميداني</h2></div><button type="button" className="r2-command-link" onClick={()=>navigate('operations')}>العمليات</button></div>
          {fields.length===0?<p className="r2-command-empty">لا توجد إسنادات قابلة لإعادة التوزيع.</p>:fields.map(a=><form className="r2-command-field" key={a.id} onSubmit={e=>{e.preventDefault();reassignField(a.id,a.version,e.currentTarget)}}><div><strong>{a.transactionType}</strong><small>المالك الحالي {a.assignedUserName??'غير محدد'} · {a.destinationLabel}</small></div><select name="member" aria-label="المالك الجديد" defaultValue=""><option value="">اختر عضوًا</option>{snapshot.field.members.map(m=><option key={m.userId} value={m.userId}>{m.displayName}</option>)}</select><input name="reason" aria-label="سبب إعادة الإسناد" placeholder="سبب موثق"/><button className="r2-action r2-action--primary" disabled={Boolean(busy)}>إعادة الإسناد</button></form>)}
        </article>
