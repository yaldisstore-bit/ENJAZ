...
function Harness(){const[ready,setReady]=useState(false);useLayoutEffect(()=>setReady(true),[]);return <div className="ez-r2-root r2-shell" data-r2-runtime-mode="live" data-destination="documents"><main id="r2-main" className="r2-shell__main" aria-label="خزنة الوثائق"><div className="r2-screen" data-records-stage="R2.0-6" data-records-domain="documents" aria-hidden="true"/></main>{ready?<LiveDocumentVaultPortal factory={async()=>gateway} workspace={Promise.resolve(W)}/>:null}</div>}
const root=document.getElementById('phase101-browser-root');if(!root)throw new Error('Phase 10.1 browser root missing');createRoot(root).render(<StrictMode><DataLayerProvider factory={factory}><Harness/></DataLayerProvider></StrictMode>);
