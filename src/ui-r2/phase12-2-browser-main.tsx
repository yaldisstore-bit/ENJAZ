...
function Harness(){const[ready,setReady]=useState(false);useLayoutEffect(()=>setReady(true),[]);return <div className="ez-r2-root r2-shell" data-r2-runtime-mode="live" data-destination="copilot"><aside className="r2-shell__rail" aria-hidden="true"/><div className="r2-shell__workspace"><main id="r2-main" className="r2-shell__main" aria-label="مساعد إنجاز"><section data-live-deferred="true">placeholder</section></main></div>{ready?<LiveCopilotPortal workspace={Promise.resolve(W)} invoke={invoke}/>:null}</div>}
const root=document.getElementById('phase122-browser-root');if(!root)throw new Error('Phase 12.2 browser root missing');createRoot(root).render(<StrictMode><Harness/></StrictMode>);
