...
export function FinancialIntelligencePanel({intelligence:x}:{readonly intelligence:FinanceIntelligenceSnapshot}){
 return <section className="r2-f73-intelligence">
  <header className="r2-f73-hero"><p>Phase 7.3 · Financial Intelligence · M13 finance anchor</p><h1>الرؤية المالية</h1></header>
  ...
  <section className="r2-f73-grid">
   ...
   <article className="r2-f73-card"><h2>الصحة المالية للشركات</h2>{x.companyHealth.length?x.companyHealth.slice(0,6).map(v=><div className="r2-f73-health" key={v.companyId}><span>{v.companyLabel} · {v.healthScore}/100</span><b>{m(v.outstandingCents)}</b></div>):<p>لا توجد بيانات مالية للشركات بعد</p>}</article>
  </section>
