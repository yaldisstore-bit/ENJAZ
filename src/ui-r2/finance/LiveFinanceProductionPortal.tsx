...
export function LiveFinanceProductionPortal(){
  const finance=useLiveRecordsPortal('finance','[data-operational-domain=finance]'),risk=useLiveRecordsPortal('risk','[data-live-deferred]'),factory=useDataLayerFactory(),fieldOperations=useFieldOperationsCommandGateway(),userId=useCurrentUserId();
  useEffect(()=>{if(!risk.active||!userId)return;const t=risk.target!;t.replaceChildren((document.getElementById('enjaz-risk-template') as HTMLTemplateElement).content.cloneNode(true));const r=t.querySelector<HTMLElement>('[data-operational-domain=risk]')!,b=r.querySelector<HTMLElement>('[data-risk-body]')!;loadSmartRisk({dataFactory:factory,fieldOperations},userId).then(x=>{r.dataset.riskEngine='smart-risk-v1';r.dataset.riskSignalCount=String(x.signals.length);let s='';for(const v of x.signals)s+=`${v.code}: ${v.explanation} [${v.evidence.map(e=>e.sourceDomain).join('، ')}]\n`;b.textContent=s||b.dataset.empty!},()=>b.textContent=b.dataset.error!)},[risk.active,factory,fieldOperations,userId]);
