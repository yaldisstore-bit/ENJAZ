...
import type { UniversalInboxItem } from '../../features/daily-work/universalInboxContract.ts';
...
function ConnectedWorkRow({ item, controller }: Readonly<{ item: UniversalInboxItem; controller: ReturnType<typeof useDailyWork> }>) {
  const busy = controller.actionItemId === item.id;
  return <article className={`r2-core-work-row is-${item.bucket}`} data-work-source={item.source} data-universal-inbox-attention={item.attention ? 'true' : 'false'} data-universal-inbox-unread={item.attention?.unread ? 'true' : 'false'}><div className="r2-core-work-row__signal"><span>{item.stateLabel}</span><strong>{item.score}</strong></div><div className="r2-core-work-row__copy">{item.attention ? <span className="r2-chip r2-chip--accent" data-inbox-attention-priority={item.attention.priority}>{item.attention.unread ? 'تنبيه جديد' : 'تنبيه'} · {priorityLabel(item.attention.priority)}</span> : null}<strong>{item.title}</strong><p>{item.subject}</p><small>{item.ownerLabel} · {dateTime(item.dueAt)}</small></div><div className="r2-core-work-row__actions">{item.completable ? <button type="button" disabled={busy} onClick={() => { void controller.complete(item); }}>{busy ? 'جارٍ التنفيذ…' : 'إكمال'}</button> : null}{item.snoozable ? <button type="button" disabled={busy} onClick={() => { void controller.snooze(item); }}>{busy ? 'جارٍ التنفيذ…' : 'تأجيل ساعتين'}</button> : null}</div></article>;
}
...
function ConnectedToday({ followupsOnly = false }: Readonly<{ followupsOnly?: boolean }>) {
  ...
  return <div className="r2-screen r2-core-today" data-screen={followupsOnly ? 'core-followups' : 'today'} data-core-connected={followupsOnly ? 'followups' : 'today'} data-phase11-2-universal-inbox={followupsOnly ? undefined : 'live'}><header className="r2-section-heading r2-section-heading--hero"><div><LiveBadge /><p className="r2-eyebrow">{followupsOnly ? 'Daily Work · متصل' : 'Phase 11.2 · Universal Inbox · متصل'}</p><h1>{followupsOnly ? 'المتابعات' : 'اليوم'}</h1><p className="r2-supporting">{followupsOnly ? 'المتابعات الموثوقة فقط؛ إكمالها وتأجيلها يمران عبر سلطة المتابعة الأصلية.' : 'عمل موحّد من المصادر الأصلية؛ إشعارات 11.1 تضيف حالة الانتباه إلى العمل المطابق من دون إنشاء نسخة ثانية أو تغيير حقيقة العمل.'}</p></div></header>{!followupsOnly ? <section className="r2-core-focus"><div><span>الأولوية الآن</span><h2>{snapshot.focus?.title ?? 'لا توجد أولوية'}</h2><p>{snapshot.focus?.subject ?? 'مساحة العمل هادئة.'}</p></div><div className="r2-core-focus__stats"><span><strong>{snapshot.summary.blocked}</strong> عائق</span><span><strong>{snapshot.summary.overdue}</strong> متأخر</span><span><strong>{snapshot.summary.dueToday}</strong> اليوم</span></div></section> : <section className="r2-core-scope-note"><strong>مركز المتابعات المتصل</strong><span>إكمال وتأجيل المتابعات يمر عبر Daily Work service داخل مساحة العمل والصلاحيات الحالية.</span></section>}<div className="r2-core-filter-strip">{(['all','overdue','today','action','upcoming'] as const).map((value) => <button key={value} type="button" aria-pressed={bucket === value} onClick={() => setBucket(value)}>{value === 'all' ? 'الكل' : value === 'overdue' ? 'متأخرة' : value === 'today' ? 'اليوم' : value === 'action' ? 'بحاجة إجراء' : 'قادمة'}</button>)}</div>{controller.actionError ? <p className="r2-core-preview-status" role="alert">{controller.actionError}</p> : null}<section className="r2-core-work-list">{filtered.length ? filtered.map((item) => <ConnectedWorkRow key={item.id} item={item} controller={controller} />) : <StatePanel title="لا توجد عناصر في هذا العرض" body="لم يتم تصنيع عناصر بديلة." />}</section></div>;
}
