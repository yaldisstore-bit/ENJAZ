  ...
  return (
    <label className={`ez-field ${error ? 'ez-field--error' : ''} ${className}`.trim()} htmlFor={fieldId}>
      ...
      <span className="ez-field__control">
        ...
        <input {...inputProps} id={fieldId} aria-label={inputProps['aria-label'] ?? label} aria-invalid={error ? true : undefined} aria-describedby={error || hint ? messageId : undefined} />
      </span>
...
export function EzSurface(props: Readonly<{ children: ReactNode; tone?: 'paper' | 'warm' | 'dark' | 'gold'; emphasis?: 'quiet' | 'raised' | 'focus'; className?: string }>) {
...
export function EzMetric(props: Readonly<{ label: string; value: string; detail?: string; tone?: 'gold' | 'dark' | 'plain' }>) {
  const { label, value, detail, tone = 'plain' } = props;
  return <div className={`ez-metric ez-metric--${tone}`}><small>{label}</small><strong>{value}</strong>{detail ? <span>{detail}</span> : null}</div>;
}

export function EzRow(props: Readonly<{ index?: string; title: string; detail?: string; meta?: ReactNode; state?: ReactNode; onClick?: () => void }>) {
  const content = <>{props.index ? <span className="ez-row__index">{props.index}</span> : null}<span className="ez-row__copy"><strong>{props.title}</strong>{props.detail ? <small>{props.detail}</small> : null}</span>{props.meta ? <span className="ez-row__meta">{props.meta}</span> : null}{props.state ? <span className="ez-row__state">{props.state}</span> : null}</>;
...
export function EzNotice(props: Readonly<{ title: string; body: string; tone?: 'info' | 'success' | 'warning' | 'danger'; action?: ReactNode }>) {
  const { title, body, tone = 'info', action } = props;
  return <div className={`ez-notice ez-notice--${tone}`} role={tone === 'danger' ? 'alert' : 'status'}><span className="ez-notice__mark" aria-hidden="true" /><span className="ez-notice__copy"><strong>{title}</strong><small>{body}</small></span>{action ? <span className="ez-notice__action">{action}</span> : null}</div>;
}
...
export function EzProgress(props: Readonly<{ value: number; label: string; detail?: string }>) {
  ...
  return <div className="ez-progress"><span className="ez-progress__head"><strong>{props.label}</strong><small>{props.detail ?? `${Math.round(value)}%`}</small></span><progress max={100} value={value} aria-label={`${props.label} ${Math.round(value)}%`} /></div>;
}
