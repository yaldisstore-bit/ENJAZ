-- ENJAZ Phase 11.6-D — display projection hardening for unified intake/contract attention.
  ...
  with intake_rows as (
    select
      ...
      jsonb_build_object(
        ...
        'attentionLabel',case
          when f.status='revoked' then 'ملغى'
          when f.status='open' and f.expires_at<=now() then 'منتهي'
          when f.status='responded' then 'وصل رد'
          else 'بانتظار الطرف الخارجي'
        end,
  ...
  approval_rows as (
    select
      ...
      jsonb_build_object(
        ...
        'attentionLabel',case
          when r.status='cancelled' or r.revoked_at is not null then 'ملغى'
          when r.status='open' and r.valid_until is not null and r.valid_until<=now() then 'منتهي'
          when b.reconciled_at is not null then 'تمت التسوية'
          when response.id is not null then 'يتطلب إجراء'
          else 'بانتظار العميل'
        end,
        ...
        'decisionLabel',case response.decision when 'approved' then 'موافقة' when 'rejected' then 'رفض' else null end,
        'communicationEvidence',false,
    ...
    select
      ...
      jsonb_build_object(
        ...
        'attentionLabel',case
          ...
          when r.status='cancelled' then 'ملغى'
          when cr.status<>'effective' or cr.expires_on is null or r.due_date<>cr.expires_on then 'تعارض يحتاج مراجعة'
          when r.due_date<(now() at time zone v_timezone)::date then 'متأخر'
          when r.due_date=(now() at time zone v_timezone)::date then 'مستحق اليوم'
          else 'قادم'
        end,
