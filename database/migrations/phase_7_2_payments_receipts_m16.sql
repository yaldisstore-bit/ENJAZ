...
-- Governing rule: payments/reversals are authoritative financial events. Browser writes
-- are accepted only through guarded RPCs with explicit workspace membership checks,
-- idempotency, immutable receipt snapshots, audit events and reversal reconciliation.
...
-- -----------------------------------------------------------------------------
-- M16 commercial engagement finance anchor. This is intentionally not a second
-- finance store: all money remains in payments / payment_reversals / ledger.
-- -----------------------------------------------------------------------------
...
-- Phase 7.2 begins before any production payment exists. Keep defensive backfills so
-- a drifted environment still migrates deterministically.
...
set
  ...
  receipt_snapshot = coalesce(receipt_snapshot, jsonb_build_object(
    'version', 1,
...
set
  ...
  reversal_ref = coalesce(reversal_ref, 'ENJ-RV-' || to_char(reversed_at at time zone 'Asia/Baghdad', 'YYYY') || '-' || lpad(reversal_serial::text, 8, '0')),
  reversal_snapshot = coalesce(reversal_snapshot, jsonb_build_object(
    'version', 1,
...
-- Direct browser mutation is deliberately removed. SELECT remains RLS-protected; all
-- Phase 7.2 finance writes go through the guarded RPCs below.
...
-- -----------------------------------------------------------------------------
-- Guarded finance command RPCs.
-- Every SECURITY DEFINER function performs its own auth.uid + workspace membership
-- authorization and has PUBLIC/anon execution revoked explicitly.
-- -----------------------------------------------------------------------------
...
begin
  ...
  where workspace_id = p_workspace_id and name = btrim(p_name) limit 1;
  if found then
  ...
  insert into public.cashbox_accounts(workspace_id, name, opening_balance, opened_at, active)
  values (p_workspace_id, btrim(p_name), p_opening_balance, now(), true)
  ...
  return jsonb_build_object(
    'cashboxes', coalesce((
      ...
      from (
        ...
        from public.cashbox_accounts c
        where c.workspace_id = p_workspace_id and c.active
      ) x
    ), '[]'::jsonb),
    ...
    'recentReceipts', coalesce((
      ...
      from (
        select p.paid_at, jsonb_build_object(
          ...
          'cashboxId', p.cashbox_id,
          'engagementId', p.engagement_id,
          'note', p.note,
          'snapshot', p.receipt_snapshot,
          'reversal', (
            select jsonb_build_object('reversalId', r.id, 'reversalRef', r.reversal_ref, 'reason', r.reason, 'reversedAt', r.reversed_at, 'snapshot', r.reversal_snapshot)
            from public.payment_reversals r where r.workspace_id = p.workspace_id and r.payment_id = p.id
          )
        ) as row_data
...
begin
  ...
  select * into v_existing from public.payments p
  where p.workspace_id = p_workspace_id and p.idempotency_key = p_idempotency_key
  limit 1;
  if found then
    if v_existing.transaction_id = p_transaction_id
      and v_existing.amount = p_amount
      and v_existing.method = p_method
      and v_existing.cashbox_id is not distinct from p_cashbox_id
      and v_existing.engagement_id is not distinct from p_engagement_id then
      return jsonb_build_object(
  ...
  select * into v_tx from public.transactions t
  where t.workspace_id = p_workspace_id and t.id = p_transaction_id
  for update;
  ...
  where c.workspace_id = p_workspace_id and c.id = v_tx.company_id and c.deleted_at is null;
  ...
  if p_cashbox_id is not null then
    select * into v_cashbox from public.cashbox_accounts c
    where c.workspace_id = p_workspace_id and c.id = p_cashbox_id and c.active;
  ...
  if p_engagement_id is not null then
    ...
    if not found or not exists (
      ...
      raise foreign_key_violation using message = 'ENJAZ_PAYMENT_ENGAGEMENT_INVALID';
    end if;
  end if;
  ...
  v_snapshot := jsonb_build_object(
    ...
    'receiptSerial', v_serial,
    'workspaceId', p_workspace_id,
    'workspaceName', v_workspace.name,
    'currency', v_workspace.currency,
    'timezone', v_workspace.timezone,
    'companyId', v_company.id,
    'companyName', coalesce(nullif(btrim(v_company.display_name), ''), v_company.legal_name),
    'companyLegalName', v_company.legal_name,
    'transactionId', v_tx.id,
    'transactionLabel', coalesce(nullif(btrim(v_tx.legacy_id), ''), v_tx.type),
    'transactionType', v_tx.type,
    'amount', p_amount::text,
    'method', p_method,
    'paidAt', p_paid_at,
    'cashboxId', p_cashbox_id,
    'cashboxName', case when p_cashbox_id is null then null else v_cashbox.name end,
    'engagementId', p_engagement_id,
    'engagementTitle', case when p_engagement_id is null then null else v_engagement.title end,
  ...
  insert into public.payments(
    ...
    idempotency_key, created_by, receipt_serial, receipt_token, receipt_snapshot, receipt_version,
    cashbox_id, engagement_id
  ) values (
    p_workspace_id, p_transaction_id, v_tx.company_id, p_amount, p_method, p_paid_at, 'posted', v_receipt_ref,
    nullif(btrim(coalesce(p_note,'')), ''), p_idempotency_key, v_actor, v_serial, gen_random_uuid(), v_snapshot, 1,
    p_cashbox_id, p_engagement_id
...
begin
  ...
  select * into v_existing from public.payment_reversals r
  where r.workspace_id = p_workspace_id and r.idempotency_key = p_idempotency_key limit 1;
  if found then
  ...
  where p.workspace_id = p_workspace_id and p.id = p_payment_id
  for update;
  if not found then raise foreign_key_violation using message = 'ENJAZ_REVERSAL_PAYMENT_INVALID'; end if;

  select * into v_existing from public.payment_reversals r
  where r.workspace_id = p_workspace_id and r.payment_id = p_payment_id limit 1;
  if found then
    if v_payment.status <> 'reversed' then
      update public.payments set status = 'reversed' where workspace_id = p_workspace_id and id = p_payment_id;
    end if;
    return jsonb_build_object('reversalId', v_existing.id, 'reversalRef', v_existing.reversal_ref, 'paymentId', v_existing.payment_id,
      'reason', v_existing.reason, 'reversedAt', v_existing.reversed_at, 'snapshot', v_existing.reversal_snapshot, 'wasDuplicate', true);
  end if;
  ...
  v_snapshot := jsonb_build_object(
    ...
    'receiptRef', v_payment.receipt_ref,
    'amount', v_payment.amount::text,
    'method', v_payment.method,
    'reason', btrim(p_reason),
    'reversedAt', now(),
    'actorUserId', v_actor,
    'receiptSnapshot', v_payment.receipt_snapshot
  );
  ...
  where workspace_id = p_workspace_id and id = p_payment_id and status = 'posted';
...
begin
  ...
  return jsonb_build_object(
    ...
    'reversal', (
      select jsonb_build_object('reversalId', r.id, 'reversalRef', r.reversal_ref, 'reason', r.reason, 'reversedAt', r.reversed_at, 'snapshot', r.reversal_snapshot)
      from public.payment_reversals r where r.workspace_id = p_workspace_id and r.payment_id = p_payment_id
    )
  );
end;
$$;
