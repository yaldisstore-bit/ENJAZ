...
-- Follow-up state is private bridge evidence only. Canonical intake truth stays in intake_submissions.
-- Portal mode delegates request creation to the existing M3 save_client_portal_request_v1 authority.
begin;
...
create table private.intake_followup_requests (
  ...
  constraint intake_followup_requests_mode_check check (
    (mode='secure_link' and token_hash is not null and portal_principal_id is null and portal_transaction_id is null and portal_request_id is null and request_kind='information')
    or
    (mode='client_portal' and token_hash is null and portal_principal_id is not null and portal_transaction_id is not null and portal_request_id is not null)
  ),
...
revoke all on table private.intake_followup_requests from public,anon,authenticated;

create or replace function private.intake_followup_token_v1(p_workspace_id uuid,p_submission_id uuid,p_idempotency_key uuid)
returns text language sql immutable security definer set search_path='' as $$
  select encode(sha256(convert_to(p_workspace_id::text||':'||p_submission_id::text||':'||p_idempotency_key::text,'UTF8')),'hex');
$$;
...
begin
  ...
  for v_key,v_value in select key,value from jsonb_each(p_patch) loop
    ...
    if jsonb_typeof(v_value)<>'string' or char_length(btrim(v_value#>>'{}'))>2400 then
...
declare
  ...
  v_token text; v_hash text; v_portal jsonb; v_fields jsonb:=coalesce(p_requested_fields,'[]'::jsonb);
begin
  ...
  if found then
    if v_existing.submission_id<>p_submission_id or v_existing.mode<>p_mode or v_existing.request_kind<>p_request_kind
    ...
    return jsonb_build_object('followupId',v_existing.id,'submissionId',v_existing.submission_id,'mode',v_existing.mode,
      'requestKind',v_existing.request_kind,'status',v_existing.status,'version',v_existing.version,'submissionVersion',v_existing.expected_submission_version,
  ...
  if v_submission.status='submitted' then
    update public.intake_submissions set status='under_review',version=version+1,updated_at=now()
    where workspace_id=p_workspace_id and id=p_submission_id returning * into v_submission;
  end if;
  ...
  insert into private.intake_followup_requests(
    ...
    portal_principal_id,portal_transaction_id,portal_request_id,expected_submission_version,expires_at,idempotency_key,requested_by
  ) values(
    p_workspace_id,p_submission_id,p_mode,p_request_kind,btrim(p_title),nullif(btrim(coalesce(p_instructions,'')),''),v_fields,v_hash,
    p_portal_principal_id,p_portal_transaction_id,p_portal_request_id,v_submission.version,now()+make_interval(hours=>p_expires_in_hours),p_idempotency_key,v_actor
  ) returning * into v_row;

  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(p_workspace_id,v_actor,'intake.followup.requested','intake_submission',p_submission_id,'Governed intake follow-up requested',
  ...
  return jsonb_build_object('followupId',v_row.id,'submissionId',v_row.submission_id,'mode',v_row.mode,'requestKind',v_row.request_kind,
    'status',v_row.status,'version',v_row.version,'submissionVersion',v_row.expected_submission_version,'portalRequestId',v_row.portal_request_id,
    'token',v_token,'expiresAt',v_row.expires_at,'wasDuplicate',false);
end; $$;
...
begin
  ...
  return jsonb_build_object(
    'publicAuthority','non_authoritative_followup_input','followupId',v_row.id,'status',v_row.status,'requestKind',v_row.request_kind,
    'title',v_row.title,'instructions',v_row.instructions,'expiresAt',v_row.expires_at,'submissionVersion',v_row.expected_submission_version,
    'form',jsonb_build_object('title',v_form.public_title,'fields',coalesce((
      ...
      from public.intake_form_fields x where x.workspace_id=v_row.workspace_id and x.form_id=v_submission.form_id and v_row.requested_fields ? x.field_key
    ),'[]'::jsonb)),
...
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare v_row private.intake_followup_requests%rowtype; v_submission public.intake_submissions%rowtype; v_patch jsonb:=coalesce(p_patch,'{}'::jsonb);
begin
  v_row:=private.require_live_intake_followup_v1(p_token);
  if v_row.status='responded' then
    if coalesce(p_finalize,false) and v_row.response_patch=v_patch then
      return jsonb_build_object('followupId',v_row.id,'submissionId',v_row.submission_id,'status','responded','submissionVersion',v_row.expected_submission_version+1,'authoritative',false,'wasDuplicate',true);
    end if;
    raise object_not_in_prerequisite_state using message='ENJAZ_INTAKE_FOLLOWUP_ALREADY_RESPONDED';
  end if;
  select * into v_submission from public.intake_submissions s where s.workspace_id=v_row.workspace_id and s.id=v_row.submission_id for update;
  ...
  if not coalesce(p_finalize,false) then
    ...
    return jsonb_build_object('followupId',v_row.id,'submissionId',v_row.submission_id,'status','open','version',v_row.version,'submissionVersion',v_submission.version,'authoritative',false,'wasDuplicate',false);
  end if;
  if v_patch='{}'::jsonb then raise invalid_parameter_value using message='ENJAZ_INTAKE_FOLLOWUP_RESPONSE_EMPTY'; end if;
  update public.intake_submissions set answers=answers||v_patch,version=version+1,updated_at=now()
  where workspace_id=v_row.workspace_id and id=v_row.submission_id returning * into v_submission;
  update private.intake_followup_requests set draft_patch='{}'::jsonb,response_patch=v_patch,status='responded',responded_at=now(),version=version+1,updated_at=now()
  where id=v_row.id returning * into v_row;
  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(v_row.workspace_id,null,'intake.followup.responded','intake_submission',v_row.submission_id,'Secure intake follow-up response merged into canonical submission',
    jsonb_build_object('followupId',v_row.id,'requestKind',v_row.request_kind,'updatedFields',(select jsonb_agg(key) from jsonb_each(v_patch)),'newSubmissionVersion',v_submission.version));
  return jsonb_build_object('followupId',v_row.id,'submissionId',v_row.submission_id,'status',v_row.status,'version',v_row.version,'submissionVersion',v_submission.version,'authoritative',false,'wasDuplicate',false);
end; $$;
...
begin
  select * into v_row from private.intake_followup_requests r where r.workspace_id=p_workspace_id and r.id=p_followup_id for update;
  if not found then raise no_data_found using message='ENJAZ_INTAKE_FOLLOWUP_NOT_FOUND'; end if;
  ...
  if v_row.request_kind='information' then
    select exists(
      select 1 from public.client_portal_messages m join public.client_portal_principals p
        on p.workspace_id=m.workspace_id and p.id=m.principal_id
  ...
  select * into v_submission from public.intake_submissions s where s.workspace_id=p_workspace_id and s.id=v_row.submission_id for update;
  ...
  if v_patch<>'{}'::jsonb then
    update public.intake_submissions set answers=answers||v_patch,version=version+1,updated_at=now()
    where workspace_id=p_workspace_id and id=v_submission.id returning * into v_submission;
  end if;
  update private.intake_followup_requests set response_patch=v_patch,status='responded',responded_at=now(),version=version+1,updated_at=now()
  where id=v_row.id returning * into v_row;
  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(p_workspace_id,v_actor,'intake.followup.portal_reconciled','intake_submission',v_submission.id,'Client Portal follow-up evidence reconciled into intake review',
    jsonb_build_object('followupId',v_row.id,'portalRequestId',v_row.portal_request_id,'requestKind',v_row.request_kind,'newSubmissionVersion',v_submission.version));
  return jsonb_build_object('followupId',v_row.id,'submissionId',v_submission.id,'status',v_row.status,'version',v_row.version,'submissionVersion',v_submission.version,'portalRequestId',v_row.portal_request_id);
end; $$;
...
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare v_actor uuid:=private.require_crm_member_v1(p_workspace_id); v_row private.intake_followup_requests%rowtype;
begin
  select * into v_row from private.intake_followup_requests r where r.workspace_id=p_workspace_id and r.id=p_followup_id for update;
  if not found then raise no_data_found using message='ENJAZ_INTAKE_FOLLOWUP_NOT_FOUND'; end if;
  if v_row.version<>p_expected_version then raise serialization_failure using message='ENJAZ_INTAKE_FOLLOWUP_STALE'; end if;
  if v_row.status='revoked' then return jsonb_build_object('followupId',v_row.id,'status','revoked','version',v_row.version,'wasDuplicate',true); end if;
  if v_row.status<>'open' then raise object_not_in_prerequisite_state using message='ENJAZ_INTAKE_FOLLOWUP_NOT_OPEN'; end if;
  if v_row.mode='client_portal' then
    perform private.revoke_client_portal_request_v1_impl(p_workspace_id,v_row.portal_principal_id,v_row.portal_request_id,null);
  end if;
  update private.intake_followup_requests set status='revoked',revoked_at=now(),version=version+1,updated_at=now()
  where id=v_row.id returning * into v_row;
  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(p_workspace_id,v_actor,'intake.followup.revoked','intake_submission',v_row.submission_id,'Governed intake follow-up revoked',jsonb_build_object('followupId',v_row.id,'mode',v_row.mode,'portalRequestId',v_row.portal_request_id));
  return jsonb_build_object('followupId',v_row.id,'status',v_row.status,'version',v_row.version,'wasDuplicate',false);
end; $$;

create or replace function public.revoke_intake_followup_v1(uuid,uuid,integer)
returns jsonb language sql volatile security invoker set search_path='' as $$
  select private.revoke_intake_followup_v1_impl($1,$2,$3);
$$;

revoke all on function private.intake_followup_token_v1(uuid,uuid,uuid) from public,anon;
revoke all on function private.validate_intake_followup_patch_v1(uuid,uuid,jsonb,jsonb) from public,anon;
revoke all on function private.require_live_intake_followup_v1(text) from public;
...
grant execute on function private.require_live_intake_followup_v1(text) to anon,authenticated,service_role;
grant execute on function private.validate_intake_followup_patch_v1(uuid,uuid,jsonb,jsonb) to anon,authenticated,service_role;
grant execute on function private.intake_followup_token_v1(uuid,uuid,uuid) to authenticated,service_role;
...
revoke all on function public.get_public_intake_followup_v1(text) from public;
grant execute on function public.get_public_intake_followup_v1(text) to anon,authenticated;
revoke all on function public.save_public_intake_followup_v1(text,jsonb,boolean) from public;
...
revoke all on function public.revoke_intake_followup_v1(uuid,uuid,integer) from public,anon,service_role;
grant execute on function public.revoke_intake_followup_v1(uuid,uuid,integer) to authenticated;
