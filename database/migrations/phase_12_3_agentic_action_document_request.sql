-- ENJAZ Phase 12.3 A3-D — governed client document-request adapter.
-- Adds exactly one fourth low-risk action-specific adapter over existing M3 authority.
-- Request type is hard-locked to document; resource share, update, payment, approval,
-- appointment, information and generic portal actions are not exposed.
...
alter table private.copilot_agent_proposals
  add column action_principal_id uuid null,
  add column action_instructions text null,
  add column action_valid_until timestamptz null;

alter table private.copilot_agent_proposals
  drop constraint if exists copilot_agent_proposals_action_shape_check;
...
alter table private.copilot_agent_proposals
  ...
  check(
    ...
    (proposal_kind='action' and action_kind='followup.create'
      ...
      and action_transaction_id is not null and action_title is not null
      and char_length(action_title) between 1 and 320 and action_due_at is not null
      and action_source_kind is null and action_operation_id is null and action_scheduled_for is null
      and action_principal_id is null and action_instructions is null and action_valid_until is null)
    or
...
create index copilot_agent_proposals_document_request_idx
  on private.copilot_agent_proposals(workspace_id,action_principal_id,action_transaction_id,action_target_id)
  where action_kind='document.request';

create or replace function private.copilot_document_request_hash_v1(
  p_workspace_id uuid,p_request_id uuid,p_principal_id uuid,p_transaction_id uuid,p_portal_request_id uuid,
  p_title text,p_instructions text,p_due_at timestamptz,p_valid_until timestamptz
)
returns text
language sql
immutable
  ...
  select encode(
    extensions.digest(
      convert_to(
        ...
        ||'|'||lower(p_principal_id::text)
        ||'|'||lower(p_transaction_id::text)
        ||'|'||lower(p_portal_request_id::text)
        ||'|'||encode(extensions.digest(convert_to(p_title,'UTF8'),'sha256'),'hex')
        ||'|'||encode(extensions.digest(convert_to(coalesce(p_instructions,''),'UTF8'),'sha256'),'hex')
        ||'|'||to_char(p_due_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
        ||'|'||to_char(p_valid_until at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
        'UTF8'
      ),
      'sha256'
    ),
...
$$;
revoke all on function private.copilot_document_request_hash_v1(uuid,uuid,uuid,uuid,uuid,text,text,timestamptz,timestamptz)
  from public,anon,authenticated,service_role;

create or replace function private.copilot_begin_request_v7_impl(
  p_workspace_id uuid,
...
create or replace function private.copilot_register_document_request_proposal_v1_impl(
  p_workspace_id uuid,p_actor_user_id uuid,p_request_id uuid,p_proposal_hash text,
  p_principal_id uuid,p_transaction_id uuid,p_portal_request_id uuid,p_title text,p_instructions text,
  p_due_at timestamptz,p_valid_until timestamptz,p_expires_at timestamptz
)
returns jsonb
language plpgsql
security definer
...
begin
  if p_workspace_id is null or p_actor_user_id is null or p_request_id is null
     or p_principal_id is null or p_transaction_id is null or p_portal_request_id is null then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_INVALID_ID';
  end if;
  ...
  if char_length(v_title) not between 1 and 320 then
    raise invalid_parameter_value using message='ENJAZ_PORTAL_REQUEST_TITLE_INVALID';
  end if;
  if v_instructions is not null and char_length(v_instructions) not between 1 and 2400 then
    raise invalid_parameter_value using message='ENJAZ_PORTAL_REQUEST_INSTRUCTIONS_INVALID';
  end if;
  if p_due_at is null or p_valid_until is null
     or p_due_at<=v_now+interval '30 seconds'
     or p_valid_until<=v_now+interval '30 seconds'
     or p_due_at>p_valid_until then
    raise invalid_parameter_value using message='ENJAZ_PORTAL_REQUEST_VALIDITY_INVALID';
  end if;
  if p_expires_at is null or p_expires_at<=v_now+interval '30 seconds' or p_expires_at>v_now+interval '30 minutes' then
  ...
  v_expected_hash:=private.copilot_document_request_hash_v1(
    p_workspace_id,p_request_id,p_principal_id,p_transaction_id,p_portal_request_id,
    v_title,v_instructions,p_due_at,p_valid_until
  );
  ...
  if found then
    if v_existing.actor_user_id<>p_actor_user_id
       ...
       or v_existing.action_kind<>'document.request'
       or v_existing.action_target_id<>p_portal_request_id
       or v_existing.action_principal_id<>p_principal_id
       or v_existing.action_transaction_id<>p_transaction_id
       or v_existing.action_title<>v_title
       or v_existing.action_instructions is distinct from v_instructions
       or v_existing.action_due_at<>p_due_at
       or v_existing.action_valid_until<>p_valid_until then
    ...
    return jsonb_build_object(
      ...
      'principalId',v_existing.action_principal_id,'transactionId',v_existing.action_transaction_id,
      'title',v_existing.action_title,'instructions',v_existing.action_instructions,
      'dueAt',v_existing.action_due_at,'validUntil',v_existing.action_valid_until,'replayed',true
    );
  end if;

  insert into private.copilot_agent_proposals(
    workspace_id,actor_user_id,request_id,proposal_hash,plan_schema,proposal_kind,
    action_kind,action_target_id,action_principal_id,action_transaction_id,action_title,
    action_instructions,action_due_at,action_valid_until,expires_at
  ) values(
    p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,'enjaz.copilot.agent.plan.v1','action',
    'document.request',p_portal_request_id,p_principal_id,p_transaction_id,v_title,
    v_instructions,p_due_at,p_valid_until,p_expires_at
  ...
  return jsonb_build_object(
    ...
    'principalId',v_row.action_principal_id,'transactionId',v_row.action_transaction_id,
    'title',v_row.action_title,'instructions',v_row.action_instructions,
    'dueAt',v_row.action_due_at,'validUntil',v_row.action_valid_until,'replayed',false
  );
end;
$$;

create or replace function private.copilot_execute_document_request_v1_impl(
...
begin
  ...
  if not exists(
    select 1 from public.workspace_memberships m where m.workspace_id=p_workspace_id and m.user_id=v_actor
  ) then
    raise insufficient_privilege using message='ENJAZ_COPILOT_WORKSPACE_FORBIDDEN';
  end if;

  perform pg_advisory_xact_lock(hashtextextended(p_workspace_id::text||':'||p_proposal_id::text||':execute',0));
  select * into v_row from private.copilot_agent_proposals p
  where p.workspace_id=p_workspace_id and p.id=p_proposal_id for update;
  ...
  if v_row.proposal_kind<>'action' or v_row.action_kind<>'document.request'
     or v_row.action_target_id is null or v_row.action_principal_id is null or v_row.action_transaction_id is null
     or v_row.action_title is null or v_row.action_due_at is null or v_row.action_valid_until is null then
  ...
  if v_row.status='consumed' then
    if v_row.execution_key=p_execution_key and v_row.consumed_by=v_actor then
      return jsonb_build_object(
        ...
        'proposalId',v_row.id,'proposalHash',v_row.proposal_hash,'executionKey',v_row.execution_key,
        'actionKind',v_row.action_kind,'targetId',v_row.action_target_id,
        'principalId',v_row.action_principal_id,'transactionId',v_row.action_transaction_id,
        'title',v_row.action_title,'instructions',v_row.action_instructions,
        'dueAt',v_row.action_due_at,'validUntil',v_row.action_valid_until,
        'requestType','document','resourceShareId',null,'result',v_row.execution_result,'replayed',true
      );
    end if;
  ...
  if v_row.action_due_at<=v_now or v_row.action_valid_until<=v_now then
    raise check_violation using message='ENJAZ_COPILOT_DOCUMENT_REQUEST_STALE';
  end if;
  if exists(
    select 1 from private.copilot_agent_proposals p
    where p.workspace_id=p_workspace_id and p.execution_key=p_execution_key and p.id<>p_proposal_id
  ) then raise unique_violation using message='ENJAZ_COPILOT_EXECUTION_REPLAY_CONFLICT'; end if;

  v_domain:=public.save_client_portal_request_v1(
    p_workspace_id,v_row.action_principal_id,v_row.action_target_id,null,
  ...
  return jsonb_build_object(
    ...
    'title',v_row.action_title,'instructions',v_row.action_instructions,
    'dueAt',v_row.action_due_at,'validUntil',v_row.action_valid_until,
    'requestType','document','resourceShareId',null,'result',v_row.execution_result,'replayed',false
  );
end;
$$;

create or replace function public.copilot_begin_request_v7(
...
as $$ select private.copilot_begin_request_v7_impl(p_workspace_id,p_actor_user_id,p_request_id,p_operation,p_payload_hash,p_limit); $$;

create or replace function public.copilot_register_document_request_proposal_v1(
  p_workspace_id uuid,p_actor_user_id uuid,p_request_id uuid,p_proposal_hash text,
  p_principal_id uuid,p_transaction_id uuid,p_portal_request_id uuid,p_title text,p_instructions text,
  p_due_at timestamptz,p_valid_until timestamptz,p_expires_at timestamptz
)
returns jsonb language sql security invoker set search_path=''
as $$ select private.copilot_register_document_request_proposal_v1_impl(
  p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,p_principal_id,p_transaction_id,p_portal_request_id,
  p_title,p_instructions,p_due_at,p_valid_until,p_expires_at
); $$;

create or replace function public.copilot_execute_document_request_v1(
  p_workspace_id uuid,p_proposal_id uuid,p_proposal_hash text,p_execution_key uuid
)
returns jsonb language sql volatile security invoker set search_path=''
as $$ select private.copilot_execute_document_request_v1_impl(p_workspace_id,p_proposal_id,p_proposal_hash,p_execution_key); $$;

revoke all on function private.copilot_begin_request_v7_impl(uuid,uuid,uuid,text,text,integer) from public,anon,authenticated;
grant execute on function private.copilot_begin_request_v7_impl(uuid,uuid,uuid,text,text,integer) to service_role;
revoke all on function public.copilot_begin_request_v7(uuid,uuid,uuid,text,text,integer) from public,anon,authenticated;
grant execute on function public.copilot_begin_request_v7(uuid,uuid,uuid,text,text,integer) to service_role;

revoke all on function private.copilot_register_document_request_proposal_v1_impl(uuid,uuid,uuid,text,uuid,uuid,uuid,text,text,timestamptz,timestamptz,timestamptz)
  from public,anon,authenticated;
grant execute on function private.copilot_register_document_request_proposal_v1_impl(uuid,uuid,uuid,text,uuid,uuid,uuid,text,text,timestamptz,timestamptz,timestamptz)
  to service_role;
revoke all on function public.copilot_register_document_request_proposal_v1(uuid,uuid,uuid,text,uuid,uuid,uuid,text,text,timestamptz,timestamptz,timestamptz)
  from public,anon,authenticated;
grant execute on function public.copilot_register_document_request_proposal_v1(uuid,uuid,uuid,text,uuid,uuid,uuid,text,text,timestamptz,timestamptz,timestamptz)
  to service_role;

revoke all on function private.copilot_execute_document_request_v1_impl(uuid,uuid,text,uuid) from public,anon,service_role;
grant execute on function private.copilot_execute_document_request_v1_impl(uuid,uuid,text,uuid) to authenticated;
revoke all on function public.copilot_execute_document_request_v1(uuid,uuid,text,uuid) from public,anon,service_role;
grant execute on function public.copilot_execute_document_request_v1(uuid,uuid,text,uuid) to authenticated;
