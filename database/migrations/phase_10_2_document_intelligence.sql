...
do $$ begin
  ...
  if not exists(select 1 from pg_constraint where conname='document_analysis_source_version_check') then
    alter table public.document_analysis add constraint document_analysis_source_version_check check(source_version_number is null or source_version_number>0);
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_source_checksum_check') then
    alter table public.document_analysis add constraint document_analysis_source_checksum_check check(source_checksum is null or source_checksum ~ '^[0-9a-f]{64}$');
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_verification_state_check') then
    alter table public.document_analysis add constraint document_analysis_verification_state_check check(verification_state in ('queued','extracting','review_required','reviewed','verified','rejected','failed','superseded','legacy_unverified'));
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_pages_array_check') then
    alter table public.document_analysis add constraint document_analysis_pages_array_check check(jsonb_typeof(page_results)='array');
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_provenance_object_check') then
    alter table public.document_analysis add constraint document_analysis_provenance_object_check check(jsonb_typeof(provenance)='object');
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_review_note_check') then
    alter table public.document_analysis add constraint document_analysis_review_note_check check(review_note is null or char_length(review_note)<=1000);
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_failure_code_check') then
    alter table public.document_analysis add constraint document_analysis_failure_code_check check(failure_code is null or char_length(btrim(failure_code)) between 1 and 160);
  end if;
  if not exists(select 1 from pg_constraint where conname='document_analysis_provider_run_id_check') then
    alter table public.document_analysis add constraint document_analysis_provider_run_id_check check(provider_run_id is null or char_length(provider_run_id)<=320);
  end if;
end $$;

with latest as (
  select distinct on(v.workspace_id,v.document_id) v.workspace_id,v.document_id,v.id as version_id,v.version_number,v.checksum
...
set document_version_id=coalesce(a.document_version_id,l.version_id),
    ...
    provenance=case when a.provenance='{}'::jsonb then jsonb_build_object('documentId',a.document_id,'documentVersionId',l.version_id,'sourceVersionNumber',l.version_number,'sourceChecksum',l.checksum,'sourceAuthority','SOURCE_FILE_REMAINS_AUTHORITATIVE','legacyBackfill',true) else a.provenance end,
...
begin
  if coalesce(auth.role(),'')<>'service_role' then raise insufficient_privilege using message='ENJAZ_DOCUMENT_INTELLIGENCE_SERVICE_REQUIRED'; end if;
end;$$;
revoke all on function private.require_document_intelligence_service_v1() from public,anon,authenticated;

create or replace function private.validate_document_extracted_fields_v1(p_fields jsonb)
...
create or replace function private.validate_document_pages_v1(p_pages jsonb)
returns void language plpgsql immutable set search_path='' as $$
declare v jsonb;v_page numeric;v_conf numeric;
begin
  ...
  if (select count(*) from jsonb_array_elements(p_pages))<>(select count(distinct (value->>'pageNumber')::integer) from jsonb_array_elements(p_pages)) then raise invalid_parameter_value using message='ENJAZ_OCR_PAGE_DUPLICATE'; end if;
end;$$;
revoke all on function private.validate_document_pages_v1(jsonb) from public,anon,authenticated;

create or replace function public.request_document_extraction_v1(p_workspace_id uuid,p_request_id uuid,p_document_id uuid,p_version_number integer default null)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_actor uuid;v_doc public.documents%rowtype;v_ver public.document_versions%rowtype;v_existing public.document_analysis%rowtype;v_analysis_version integer;
begin
  ...
  values(p_request_id,p_workspace_id,p_document_id,v_ver.id,v_ver.version_number,v_ver.checksum,v_analysis_version,null,'{}'::jsonb,null,null,'unreviewed','queued','[]'::jsonb,jsonb_build_object('documentId',p_document_id,'documentVersionId',v_ver.id,'sourceVersionNumber',v_ver.version_number,'sourceChecksum',v_ver.checksum,'mimeType',v_ver.mime_type,'sizeBytes',v_ver.size_bytes,'sourceAuthority','SOURCE_FILE_REMAINS_AUTHORITATIVE','extractedContentMayReplaceSource',false),v_actor,null,now(),now());
...
begin
  ...
  if not found then raise no_data_found using message='ENJAZ_OCR_ANALYSIS_NOT_FOUND'; end if;
  if v.verification_state='extracting' then return jsonb_build_object('schema','enjaz.document-extraction-state.v1','analysisId',v.id,'state','extracting','wasDuplicate',true); end if;
  if v.verification_state<>'queued' then raise serialization_failure using message='ENJAZ_OCR_TRANSITION_INVALID'; end if;
  update public.document_analysis set verification_state='extracting',provider=v_provider,provider_run_id=p_provider_run_id,updated_at=now() where id=v.id;
  return jsonb_build_object('schema','enjaz.document-extraction-state.v1','analysisId',v.id,'state','extracting','wasDuplicate',false);
end;$$;

create or replace function public.complete_document_extraction_v1(p_analysis_id uuid,p_ocr_text text,p_extracted_fields jsonb,p_page_results jsonb,p_classification text,p_confidence numeric,p_provider text,p_provider_run_id text default null)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v public.document_analysis%rowtype;v_latest public.document_versions%rowtype;v_stale boolean;v_provider text:=btrim(coalesce(p_provider,''));
begin
  perform private.require_document_intelligence_service_v1();
  if p_confidence is null or p_confidence<0 or p_confidence>1 then raise invalid_parameter_value using message='ENJAZ_OCR_CONFIDENCE_INVALID'; end if;
  if char_length(coalesce(p_ocr_text,''))>8000000 then raise invalid_parameter_value using message='ENJAZ_OCR_TEXT_TOO_LARGE'; end if;
  if char_length(v_provider) not between 1 and 240 then raise invalid_parameter_value using message='ENJAZ_OCR_PROVIDER_INVALID'; end if;
  perform private.validate_document_extracted_fields_v1(p_extracted_fields);perform private.validate_document_pages_v1(p_page_results);
  select * into v from public.document_analysis a where a.id=p_analysis_id for update;
  if not found then raise no_data_found using message='ENJAZ_OCR_ANALYSIS_NOT_FOUND'; end if;
  ...
  select * into v_latest from public.document_versions d where d.workspace_id=v.workspace_id and d.document_id=v.document_id order by d.version_number desc limit 1;
  v_stale:=not found or v_latest.id<>v.document_version_id;
  update public.document_analysis set ocr_text=p_ocr_text,extracted_fields=p_extracted_fields,page_results=p_page_results,classification=nullif(btrim(coalesce(p_classification,'')),''),confidence=p_confidence,provider=v_provider,provider_run_id=p_provider_run_id,verification_state=case when v_stale then 'superseded' else 'review_required' end,review_status='unreviewed',failure_code=null,failure_message=null,provenance=provenance||jsonb_build_object('provider',v_provider,'providerRunId',p_provider_run_id,'completedAt',now(),'staleAtCompletion',v_stale),updated_at=now() where id=v.id;
  return jsonb_build_object('schema','enjaz.document-extraction-state.v1','analysisId',v.id,'state',case when v_stale then 'superseded' else 'review_required' end,'stale',v_stale,'sourceAuthoritative',true);
end;$$;
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v public.document_analysis%rowtype;v_code text:=btrim(coalesce(p_failure_code,''));
begin
  perform private.require_document_intelligence_service_v1();
  if char_length(v_code) not between 1 and 160 then raise invalid_parameter_value using message='ENJAZ_OCR_FAILURE_CODE_INVALID'; end if;
  select * into v from public.document_analysis a where a.id=p_analysis_id for update;
  if not found then raise no_data_found using message='ENJAZ_OCR_ANALYSIS_NOT_FOUND'; end if;
...
create or replace function public.review_document_extraction_v1(p_workspace_id uuid,p_analysis_id uuid,p_decision text,p_corrected_fields jsonb default null,p_note text default null)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  select * into v from public.document_analysis a where a.workspace_id=p_workspace_id and a.id=p_analysis_id for update;
  if not found then raise no_data_found using message='ENJAZ_OCR_ANALYSIS_NOT_FOUND'; end if;
  if v.verification_state='verified' then return jsonb_build_object('schema','enjaz.document-extraction-state.v1','analysisId',v.id,'state','verified','verified',true,'wasDuplicate',true); end if;
  ...
  select * into v_latest from public.document_versions d where d.workspace_id=v.workspace_id and d.document_id=v.document_id order by d.version_number desc limit 1;
  if not found or v_latest.id<>v.document_version_id then
    update public.document_analysis set verification_state='superseded',updated_at=now() where id=v.id;
    return jsonb_build_object('schema','enjaz.document-extraction-state.v1','analysisId',v.id,'state','superseded','verified',false,'reason','SOURCE_VERSION_CHANGED');
  end if;
  update public.document_analysis set verification_state='verified',verified_by=v_actor,verified_at=now(),review_note=case when p_note is null then review_note else concat_ws(E'\n',review_note,p_note) end,updated_at=now() where id=v.id;
  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
...
begin
  ...
  if not found then raise no_data_found using message='ENJAZ_OCR_DOCUMENT_NOT_FOUND'; end if;
  select * into v_current from public.document_versions v where v.workspace_id=p_workspace_id and v.document_id=p_document_id order by v.version_number desc limit 1;
  select coalesce(jsonb_agg(jsonb_build_object('id',a.id,'documentId',a.document_id,'documentVersionId',a.document_version_id,'sourceVersionNumber',a.source_version_number,'analysisVersion',a.analysis_version,'state',a.verification_state,'reviewStatus',a.review_status,'ocrText',a.ocr_text,'extractedFields',a.extracted_fields,'pages',a.page_results,'classification',a.classification,'confidence',a.confidence,'provider',a.provider,'stale',(v_current.id is null or a.document_version_id is null or a.document_version_id<>v_current.id),'analyzedAt',a.analyzed_at,'reviewedAt',a.reviewed_at,'verifiedAt',a.verified_at,'failureCode',a.failure_code) order by a.analysis_version desc),'[]'::jsonb) into v_rows
  from public.document_analysis a where a.workspace_id=p_workspace_id and a.document_id=p_document_id;
  return jsonb_build_object('schema','enjaz.document-intelligence.v1','documentId',p_document_id,'currentVersionId',v_current.id,'currentVersionNumber',v_current.version_number,'sourceAuthority','SOURCE_FILE_REMAINS_AUTHORITATIVE','analyses',v_rows);
end;$$;
