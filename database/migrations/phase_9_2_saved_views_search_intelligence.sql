...
create table public.saved_views (
  ...
  constraint saved_views_definition_schema_check check (
    jsonb_typeof(definition->'schema')='string'
    and definition->>'schema'='enjaz.saved-view.v1'
  ),
  constraint saved_views_definition_domain_check check (
...
create index saved_views_owner_lookup_idx
  ...
  where deleted_at is null;
create index saved_views_team_fk_idx
  on public.saved_views(workspace_id,team_id)
...
begin
  ...
  if exists(
    select 1
    from jsonb_object_keys(p_definition) k(key)
    where k.key not in ('schema','domain','query','filters','sort','dateRange','pageSize','sourceSchema')
  ) then return false; end if;

  if jsonb_typeof(p_definition->'schema')<>'string'
     or p_definition->>'schema'<>'enjaz.saved-view.v1' then return false; end if;
  if jsonb_typeof(p_definition->'domain')<>'string'
     or p_definition->>'domain' not in ('transactions','companies','people','procedures','documents') then return false; end if;
  if jsonb_typeof(p_definition->'query')<>'string'
     or char_length(p_definition->>'query')>120 then return false; end if;
  ...
  for v_filter in select key,value from jsonb_each(p_definition->'filters') loop
    ...
    if jsonb_typeof(v_filter.value)='string'
       and char_length(v_filter.value #>> '{}')>120 then return false; end if;
  end loop;

  if jsonb_typeof(p_definition->'sort') not in ('string','null') then return false; end if;
  if jsonb_typeof(p_definition->'sort')='string'
     and char_length(p_definition->>'sort')>64 then return false; end if;
  ...
  if exists(
    select 1
    from jsonb_object_keys(p_definition->'dateRange') k(key)
    where k.key not in ('from','to')
  ) then return false; end if;
  ...
  v_from:=case when jsonb_typeof(p_definition->'dateRange'->'from')='null'
               then null else p_definition->'dateRange'->>'from' end;
  v_to:=case when jsonb_typeof(p_definition->'dateRange'->'to')='null'
             then null else p_definition->'dateRange'->>'to' end;
  if not private.is_saved_view_iso_date_v1(v_from)
     or not private.is_saved_view_iso_date_v1(v_to) then return false; end if;
  ...
  if jsonb_typeof(p_definition->'sourceSchema')='string'
     and char_length(p_definition->>'sourceSchema')>96 then return false; end if;

  -- Search results/entities are never persistence payloads. Exact top-level keys above
  -- already reject these, and these markers are retained as an explicit safety invariant.
...
begin
  if p_visibility='personal' then
    if p_team_id is not null then
      raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_PERSONAL_TEAM_FORBIDDEN';
    end if;
    return;
  end if;
  ...
  if p_visibility='team' then
    if p_team_id is null then
      raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_TEAM_REQUIRED';
    end if;
  ...
  if p_visibility='workspace' then
    if p_team_id is not null then
      raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_WORKSPACE_TEAM_FORBIDDEN';
    end if;
...
begin
  ...
  if p_operation_id is null then
    raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_OPERATION_REQUIRED';
  end if;
  if v_name is null or char_length(v_name)<1 or char_length(v_name)>80 then
    raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_NAME_INVALID';
  end if;
  if not private.validate_saved_view_definition_v1(p_definition) then
    raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_DEFINITION_INVALID';
  end if;
  ...
  if p_saved_view_id is null then
    ...
    if found then
      if v_existing.deleted_at is null
         ...
         and v_existing.visibility=p_visibility
         and v_existing.team_id is not distinct from p_team_id
         and v_existing.definition=p_definition then
        return jsonb_build_object(
          'savedViewId',v_existing.id,
    ...
    insert into public.saved_views(
      workspace_id,owner_user_id,name,domain,visibility,team_id,definition,operation_id
    ) values(
      p_workspace_id,v_actor,v_name,v_domain,p_visibility,p_team_id,p_definition,p_operation_id
    ...
    if v_row.owner_user_id<>v_actor then
      raise insufficient_privilege using message='ENJAZ_SAVED_VIEW_OWNER_REQUIRED';
    end if;

    if v_row.operation_id=p_operation_id then
      if v_row.name=v_name
         and v_row.visibility=p_visibility
         and v_row.team_id is not distinct from p_team_id
         and v_row.definition=p_definition then
        return jsonb_build_object(
          'savedViewId',v_row.id,
    ...
    if p_expected_version is null or p_expected_version<>v_row.version then
      raise serialization_failure using message='ENJAZ_SAVED_VIEW_STALE';
    end if;
    if exists(
      select 1 from public.saved_views s
      where s.workspace_id=p_workspace_id
        and s.owner_user_id=v_actor
        and s.operation_id=p_operation_id
        and s.id<>v_row.id
  ...
  insert into public.audit_events(
    ...
    'saved_view',
    v_row.id,
    'Saved operational view changed',
    jsonb_build_object(
      'domain',v_row.domain,
...
returns jsonb language sql security invoker set search_path='' as $$
  select private.save_saved_view_v1_impl(
    p_workspace_id,p_saved_view_id,p_expected_version,p_operation_id,
    p_name,p_visibility,p_team_id,p_definition
  );
$$;
...
begin
  v_actor:=private.require_organization_actor_v1(p_workspace_id);
  if p_saved_view_id is null or p_operation_id is null then
    raise invalid_parameter_value using message='ENJAZ_SAVED_VIEW_DELETE_INPUT_INVALID';
  end if;
  ...
  if v_row.owner_user_id<>v_actor
     and not (
       v_row.visibility in ('team','workspace')
       and private.is_organization_owner_v1(p_workspace_id)
     ) then
  ...
  insert into public.audit_events(
    ...
    'saved_view',
    v_row.id,
    'Saved operational view deleted',
    jsonb_build_object(
      'domain',v_row.domain,
      'visibility',v_row.visibility,
      'teamId',v_row.team_id,
...
returns jsonb language sql security invoker set search_path='' as $$
  select private.delete_saved_view_v1_impl(
    p_workspace_id,p_saved_view_id,p_expected_version,p_operation_id
  );
$$;
...
-- Functions are executable by PUBLIC by default in PostgreSQL. Keep private helpers
-- private, and expose only the two guarded mutation implementations required by the
-- SECURITY INVOKER wrappers plus the explicit public API functions.
revoke execute on function private.is_saved_view_iso_date_v1(text) from public,anon,authenticated;
revoke execute on function private.validate_saved_view_definition_v1(jsonb) from public,anon,authenticated;
revoke execute on function private.can_read_saved_view_v1(uuid,uuid,text,uuid) from public,anon,authenticated;
revoke execute on function private.require_saved_view_share_authority_v1(uuid,text,uuid) from public,anon,authenticated;
...
revoke execute on function private.delete_saved_view_v1_impl(uuid,uuid,integer,uuid) from public,anon,authenticated;

grant execute on function private.can_read_saved_view_v1(uuid,uuid,text,uuid) to authenticated;
grant execute on function private.save_saved_view_v1_impl(uuid,uuid,integer,uuid,text,text,uuid,jsonb) to authenticated;
grant execute on function private.delete_saved_view_v1_impl(uuid,uuid,integer,uuid) to authenticated;
...
revoke execute on function public.delete_saved_view_v1(uuid,uuid,integer,uuid) from public,anon,authenticated;
revoke execute on function public.list_saved_views_v1(uuid) from public,anon,authenticated;
...
-- No legacy workforce insertion into workspace_memberships exists here.
