...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_manifest->'items') loop
    ...
    if exists(select 1 from public.contacts c where c.id=v_target_id)
       or exists(select 1 from public.companies c where c.id=v_target_id)
  ...
  if found then
    if v_existing.id<>p_batch_id or v_existing.reconciliation->>'payloadHash'<>v_payload_hash then
      raise serialization_failure using message='ENJAZ_LEGACY_IMPORT_IDEMPOTENCY_CONFLICT';
    end if;
    if v_existing.status<>'succeeded' then raise serialization_failure using message='ENJAZ_LEGACY_IMPORT_EXISTING_NOT_FINAL'; end if;
