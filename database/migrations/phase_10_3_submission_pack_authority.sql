...
-- A pack is an immutable composition request over already-final official drafts.
begin;
...
create table if not exists public.document_submission_packs (
  ...
  constraint document_submission_packs_title_check check (char_length(title) between 1 and 320),
  constraint document_submission_packs_manifest_check check (manifest_checksum ~ '^[0-9a-f]{64}$'),
  constraint document_submission_packs_output_consistency check (
    (status='succeeded' and output_document_id is not null and output_document_version_id is not null and failure_code is null)
    or (status<>'succeeded' and output_document_id is null and output_document_version_id is null)
  )
);
...
create table if not exists public.document_submission_pack_items (
  ...
  source_fact_snapshot_checksum text not null,
  primary key(pack_id,position),
  unique(pack_id,draft_id),
  constraint document_submission_pack_items_content_checksum check (source_content_checksum ~ '^[0-9a-f]{64}$'),
  constraint document_submission_pack_items_fact_checksum check (source_fact_snapshot_checksum ~ '^[0-9a-f]{64}$')
);
...
create or replace function public.request_document_submission_pack_v1(
  p_workspace_id uuid,
  p_request_id uuid,
...
declare
  ...
  v_existing public.document_submission_packs%rowtype;
...
begin
  ...
  if found then
    ...
    if v_existing.workspace_id<>p_workspace_id or v_existing.transaction_id<>p_transaction_id or v_existing.title<>v_title or v_existing_ids is distinct from p_draft_ids then
      raise serialization_failure using message='ENJAZ_SUBMISSION_PACK_REQUEST_DRIFT';
    end if;
  ...
  foreach v_id in array p_draft_ids loop
    ...
    if not found or v_draft.status<>'final' or v_draft.transaction_id is distinct from p_transaction_id or v_draft.final_document_id is null or v_draft.final_document_version_id is null then
      raise check_violation using message='ENJAZ_SUBMISSION_PACK_FINAL_TRANSACTION_DRAFT_REQUIRED';
    end if;
    if not exists(select 1 from public.document_versions dv where dv.workspace_id=p_workspace_id and dv.id=v_draft.final_document_version_id and dv.document_id=v_draft.final_document_id and lower(dv.mime_type)='application/pdf') then
      raise check_violation using message='ENJAZ_SUBMISSION_PACK_PDF_VERSION_REQUIRED';
    end if;
    ...
    values(p_request_id,p_workspace_id,v_position,v_draft.id,v_draft.final_document_id,v_draft.final_document_version_id,v_draft.content_checksum,v_draft.fact_snapshot_checksum);
  end loop;

  select encode(extensions.digest(convert_to(jsonb_agg(jsonb_build_object('position',i.position,'draftId',i.draft_id,'documentId',i.document_id,'documentVersionId',i.document_version_id,'contentChecksum',i.source_content_checksum,'factSnapshotChecksum',i.source_fact_snapshot_checksum) order by i.position)::text,'UTF8'),'sha256'),'hex')
    into v_manifest from public.document_submission_pack_items i where i.pack_id=p_request_id;

  insert into public.document_submission_packs(id,workspace_id,transaction_id,title,status,requested_by,manifest_checksum)
  values(p_request_id,p_workspace_id,p_transaction_id,v_title,'queued',v_actor,v_manifest);

  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(p_workspace_id,v_actor,'document.submission_pack.requested','document_submission_pack',p_request_id,'Submission pack requested from finalized transaction documents',jsonb_build_object('transactionId',p_transaction_id,'itemCount',array_length(p_draft_ids,1),'manifestChecksum',v_manifest));
...
begin
  ...
  return jsonb_build_object(
    'schema','enjaz.document-submission-pack.v1','packId',v_pack.id,'transactionId',v_pack.transaction_id,'title',v_pack.title,'status',v_pack.status,
    'requestedBy',v_pack.requested_by,'manifestChecksum',v_pack.manifest_checksum,'outputDocumentId',v_pack.output_document_id,'outputDocumentVersionId',v_pack.output_document_version_id,
    'failureCode',v_pack.failure_code,'createdAt',v_pack.created_at,'updatedAt',v_pack.updated_at,
...
begin
  ...
  if char_length(v_run) not between 1 and 200 then raise invalid_parameter_value using message='ENJAZ_SUBMISSION_PACK_SERVICE_RUN_INVALID'; end if;
  select * into v_pack from public.document_submission_packs p where p.id=p_pack_id for update;
  if not found then raise no_data_found using message='ENJAZ_SUBMISSION_PACK_NOT_FOUND'; end if;
  ...
  update public.document_submission_packs set status='running',service_run_id=v_run,updated_at=now() where id=v_pack.id;
  return jsonb_build_object('packId',v_pack.id,'status','running','wasDuplicate',false);
end;$$;
...
begin
  ...
  select * into v_pack from public.document_submission_packs p where p.id=p_pack_id for update;
  if not found then raise no_data_found using message='ENJAZ_SUBMISSION_PACK_NOT_FOUND'; end if;
  if v_pack.status='succeeded' then
    if v_pack.output_document_id=p_document_id and v_pack.output_document_version_id=p_document_version_id and v_pack.manifest_checksum=p_manifest_checksum then return jsonb_build_object('packId',v_pack.id,'status','succeeded','wasDuplicate',true); end if;
    raise serialization_failure using message='ENJAZ_SUBMISSION_PACK_COMPLETION_DRIFT';
  end if;
  ...
  insert into public.audit_events(workspace_id,actor_user_id,action,entity_type,entity_id,summary,details)
  values(v_pack.workspace_id,v_pack.requested_by,'document.submission_pack.completed','document_submission_pack',v_pack.id,'Submission pack rendered into Document Vault',jsonb_build_object('documentId',p_document_id,'documentVersionId',p_document_version_id,'manifestChecksum',p_manifest_checksum));
...
begin
  perform private.require_document_render_service_v1();
  if char_length(v_code) not between 1 and 160 then v_code:='SUBMISSION_PACK_FAILED'; end if;
  select * into v_pack from public.document_submission_packs p where p.id=p_pack_id for update;
  if not found then raise no_data_found using message='ENJAZ_SUBMISSION_PACK_NOT_FOUND'; end if;
  if v_pack.status='succeeded' then raise serialization_failure using message='ENJAZ_SUBMISSION_PACK_ALREADY_SUCCEEDED'; end if;
  if v_pack.status='failed' then return jsonb_build_object('packId',v_pack.id,'status','failed','wasDuplicate',true); end if;
  ...
  update public.document_submission_packs set status='failed',failure_code=v_code,updated_at=now() where id=v_pack.id;
  return jsonb_build_object('packId',v_pack.id,'status','failed','wasDuplicate',false);
end;$$;
