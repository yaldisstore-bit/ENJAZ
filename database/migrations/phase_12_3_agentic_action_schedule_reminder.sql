...
-- Adds exactly one third low-risk adapter over existing M10 scheduling authority.
-- Recipient is always auth.uid(); escalation and follow-up side effects are forbidden.
...
alter table private.copilot_agent_proposals
  add column action_source_kind text null,
  add column action_operation_id uuid null,
  add column action_scheduled_for timestamptz null;
...
create index copilot_agent_proposals_reminder_source_idx
  on private.copilot_agent_proposals(workspace_id,action_source_kind,action_target_id)
  where action_kind='reminder.schedule';

create or replace function private.copilot_schedule_reminder_hash_v1(
  p_workspace_id uuid,
  p_request_id uuid,
  p_source_kind text,
  p_source_id uuid,
  ...
  select encode(
    extensions.digest(
      convert_to(
        ...
        ||'|prepare_schedule_reminder'
        ||'|'||p_source_kind
        ||'|'||lower(p_source_id::text)
        ||'|'||lower(p_operation_id::text)
        ||'|'||to_char(p_scheduled_for at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
        'UTF8'
      ),
      'sha256'
    ),
...
revoke all on function private.copilot_schedule_reminder_hash_v1(uuid,uuid,text,uuid,uuid,timestamptz)
  from public,anon,authenticated,service_role;

create or replace function private.copilot_begin_request_v6_impl(
  p_workspace_id uuid,
...
create or replace function private.copilot_register_schedule_reminder_proposal_v1_impl(
  p_workspace_id uuid,
  p_actor_user_id uuid,
  p_request_id uuid,
...
begin
  if p_workspace_id is null or p_actor_user_id is null or p_request_id is null
     or p_source_id is null or p_operation_id is null then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_INVALID_ID';
  end if;
  if p_source_kind not in ('workflow_deadline','renewal_occurrence') then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_REMINDER_SOURCE_KIND_INVALID';
  end if;
  ...
  if p_scheduled_for is null or p_scheduled_for<=v_now+interval '30 seconds' then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_REMINDER_SCHEDULE_INVALID';
  end if;
  ...
  v_expected_hash:=private.copilot_schedule_reminder_hash_v1(
    p_workspace_id,p_request_id,p_source_kind,p_source_id,p_operation_id,p_scheduled_for
  );
  ...
  if found then
    if v_existing.actor_user_id<>p_actor_user_id
       ...
       or v_existing.action_kind<>'reminder.schedule'
       or v_existing.action_target_id<>p_source_id
       or v_existing.action_source_kind<>p_source_kind
       or v_existing.action_operation_id<>p_operation_id
       or v_existing.action_scheduled_for<>p_scheduled_for then
    ...
    return jsonb_build_object(
      ...
      'sourceKind',v_existing.action_source_kind,'operationId',v_existing.action_operation_id,
      'scheduledFor',v_existing.action_scheduled_for,'replayed',true
    );
  end if;

  insert into private.copilot_agent_proposals(
    workspace_id,actor_user_id,request_id,proposal_hash,plan_schema,proposal_kind,
    action_kind,action_target_id,action_source_kind,action_operation_id,action_scheduled_for,expires_at
  ) values(
    p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,'enjaz.copilot.agent.plan.v1','action',
    'reminder.schedule',p_source_id,p_source_kind,p_operation_id,p_scheduled_for,p_expires_at
  ...
  return jsonb_build_object(
    ...
    'sourceKind',v_row.action_source_kind,'operationId',v_row.action_operation_id,
    'scheduledFor',v_row.action_scheduled_for,'replayed',false
  );
end;
$$;

create or replace function private.copilot_execute_schedule_reminder_v1_impl(
  p_workspace_id uuid,
  p_proposal_id uuid,
  p_proposal_hash text,
...
begin
  ...
  if v_row.proposal_kind<>'action'
     or v_row.action_kind<>'reminder.schedule'
     or v_row.action_target_id is null
     or v_row.action_source_kind not in ('workflow_deadline','renewal_occurrence')
     or v_row.action_operation_id is null
     or v_row.action_scheduled_for is null then
  ...
  if v_row.status='consumed' then
    if v_row.execution_key=p_execution_key and v_row.consumed_by=v_actor then
      return jsonb_build_object(
        ...
        'targetId',v_row.action_target_id,'sourceKind',v_row.action_source_kind,
        'operationId',v_row.action_operation_id,'scheduledFor',v_row.action_scheduled_for,
  ...
  if v_row.action_scheduled_for<=v_now then
    raise check_violation using message='ENJAZ_COPILOT_REMINDER_SCHEDULE_STALE';
  end if;
  if exists(
  ...
  v_domain:=public.dispatch_scheduling_attention_v1(
    p_workspace_id,v_row.action_source_kind,v_row.action_target_id,v_row.action_operation_id,
...
create or replace function public.copilot_begin_request_v6(
...
as $$ select private.copilot_begin_request_v6_impl(
  p_workspace_id,p_actor_user_id,p_request_id,p_operation,p_payload_hash,p_limit
); $$;

create or replace function public.copilot_register_schedule_reminder_proposal_v1(
  p_workspace_id uuid,p_actor_user_id uuid,p_request_id uuid,p_proposal_hash text,
  p_source_kind text,p_source_id uuid,p_operation_id uuid,p_scheduled_for timestamptz,p_expires_at timestamptz
)
returns jsonb language sql security invoker set search_path=''
as $$ select private.copilot_register_schedule_reminder_proposal_v1_impl(
  p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,
  p_source_kind,p_source_id,p_operation_id,p_scheduled_for,p_expires_at
); $$;

create or replace function public.copilot_execute_schedule_reminder_v1(
  p_workspace_id uuid,p_proposal_id uuid,p_proposal_hash text,p_execution_key uuid
)
returns jsonb language sql volatile security invoker set search_path=''
as $$ select private.copilot_execute_schedule_reminder_v1_impl(
  p_workspace_id,p_proposal_id,p_proposal_hash,p_execution_key
); $$;

revoke all on function private.copilot_begin_request_v6_impl(uuid,uuid,uuid,text,text,integer)
  from public,anon,authenticated,service_role;
grant execute on function private.copilot_begin_request_v6_impl(uuid,uuid,uuid,text,text,integer)
  to service_role;

revoke all on function public.copilot_begin_request_v6(uuid,uuid,uuid,text,text,integer)
  from public,anon,authenticated;
grant execute on function public.copilot_begin_request_v6(uuid,uuid,uuid,text,text,integer)
  to service_role;

revoke all on function private.copilot_register_schedule_reminder_proposal_v1_impl(uuid,uuid,uuid,text,text,uuid,uuid,timestamptz,timestamptz)
  from public,anon,authenticated,service_role;
grant execute on function private.copilot_register_schedule_reminder_proposal_v1_impl(uuid,uuid,uuid,text,text,uuid,uuid,timestamptz,timestamptz)
  to service_role;

revoke all on function public.copilot_register_schedule_reminder_proposal_v1(uuid,uuid,uuid,text,text,uuid,uuid,timestamptz,timestamptz)
  from public,anon,authenticated;
grant execute on function public.copilot_register_schedule_reminder_proposal_v1(uuid,uuid,uuid,text,text,uuid,uuid,timestamptz,timestamptz)
  to service_role;

revoke all on function private.copilot_execute_schedule_reminder_v1_impl(uuid,uuid,text,uuid)
  from public,anon,service_role;
grant execute on function private.copilot_execute_schedule_reminder_v1_impl(uuid,uuid,text,uuid)
  to authenticated;

revoke all on function public.copilot_execute_schedule_reminder_v1(uuid,uuid,text,uuid)
  from public,anon,service_role;
grant execute on function public.copilot_execute_schedule_reminder_v1(uuid,uuid,text,uuid)
  to authenticated;
