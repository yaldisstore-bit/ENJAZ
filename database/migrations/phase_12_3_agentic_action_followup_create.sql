-- ENJAZ Phase 12.3 A3-B — action-specific follow-up creation.
-- Adds exactly one second low-risk adapter. No generic execution authority.
...
alter table private.copilot_agent_proposals
  add column action_transaction_id uuid null,
  add column action_title text null,
  add column action_due_at timestamptz null;
  ...
  add constraint copilot_agent_proposals_action_shape_check
    check(
      (proposal_kind='plan'
        ...
        and action_transaction_id is null
        and action_title is null
        and action_due_at is null)
      or
      ...
      or
      (proposal_kind='action'
        and action_kind='followup.create'
        and action_target_id is not null
        and action_snoozed_until is null
        and action_transaction_id is not null
        and action_title is not null
        and char_length(action_title) between 1 and 320
        and action_due_at is not null)
    );

create index copilot_agent_proposals_action_transaction_idx
  on private.copilot_agent_proposals(workspace_id,action_transaction_id)
  where action_kind='followup.create';

create or replace function private.copilot_followup_create_hash_v1(
  p_workspace_id uuid,
  p_request_id uuid,
...
revoke all on function private.copilot_followup_create_hash_v1(uuid,uuid,uuid,uuid,text,timestamptz)
  from public,anon,authenticated,service_role;

create or replace function private.copilot_begin_request_v5_impl(
  p_workspace_id uuid,
...
create or replace function private.copilot_register_followup_create_proposal_v1_impl(
  p_workspace_id uuid,
  p_actor_user_id uuid,
  p_request_id uuid,
...
begin
  if p_workspace_id is null or p_actor_user_id is null or p_request_id is null
     or p_transaction_id is null or p_followup_id is null then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_INVALID_ID';
  end if;
  ...
  if p_title is null or p_title<>btrim(p_title) or char_length(p_title) not between 1 and 320 then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_ACTION_TITLE_INVALID';
  end if;
  if p_due_at is null or p_due_at<=v_now+interval '30 seconds' then
    raise invalid_parameter_value using message='ENJAZ_COPILOT_ACTION_DUE_AT_INVALID';
  end if;
  ...
  v_expected_hash:=private.copilot_followup_create_hash_v1(
    p_workspace_id,p_request_id,p_transaction_id,p_followup_id,p_title,p_due_at
  );
  ...
  if found then
    if v_existing.actor_user_id<>p_actor_user_id
       ...
       or v_existing.action_kind<>'followup.create'
       or v_existing.action_target_id<>p_followup_id
       or v_existing.action_transaction_id<>p_transaction_id
       or v_existing.action_title<>p_title
       or v_existing.action_due_at<>p_due_at then
    ...
    return jsonb_build_object(
      ...
      'transactionId',v_existing.action_transaction_id,'title',v_existing.action_title,
      'dueAt',v_existing.action_due_at,'replayed',true
    );
  end if;

  insert into private.copilot_agent_proposals(
    workspace_id,actor_user_id,request_id,proposal_hash,plan_schema,proposal_kind,
    action_kind,action_target_id,action_transaction_id,action_title,action_due_at,expires_at
  ) values(
    p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,'enjaz.copilot.agent.plan.v1','action',
    'followup.create',p_followup_id,p_transaction_id,p_title,p_due_at,p_expires_at
  ...
  return jsonb_build_object(
    ...
    'transactionId',v_row.action_transaction_id,'title',v_row.action_title,
    'dueAt',v_row.action_due_at,'replayed',false
  );
end;
$$;

create or replace function private.copilot_execute_followup_create_v1_impl(
  p_workspace_id uuid,
  p_proposal_id uuid,
  p_proposal_hash text,
...
begin
  ...
  if v_row.proposal_kind<>'action'
     or v_row.action_kind<>'followup.create'
     or v_row.action_target_id is null
     or v_row.action_transaction_id is null
     or v_row.action_title is null
     or v_row.action_due_at is null then
  ...
  if v_row.status='consumed' then
    if v_row.execution_key=p_execution_key and v_row.consumed_by=v_actor then
      return jsonb_build_object(
        ...
        'targetId',v_row.action_target_id,'transactionId',v_row.action_transaction_id,
        'title',v_row.action_title,'dueAt',v_row.action_due_at,
  ...
  if v_row.action_due_at<=v_now then
    raise check_violation using message='ENJAZ_COPILOT_ACTION_DUE_AT_STALE';
  end if;
  if exists(
  ...
  v_domain:=public.create_transaction_followup_v1(
    p_workspace_id,v_row.action_transaction_id,v_row.action_target_id,v_row.action_title,v_row.action_due_at
  );
...
create or replace function public.copilot_begin_request_v5(
...
as $$ select private.copilot_begin_request_v5_impl(
  p_workspace_id,p_actor_user_id,p_request_id,p_operation,p_payload_hash,p_limit
); $$;

create or replace function public.copilot_register_followup_create_proposal_v1(
  p_workspace_id uuid,p_actor_user_id uuid,p_request_id uuid,p_proposal_hash text,
  p_transaction_id uuid,p_followup_id uuid,p_title text,p_due_at timestamptz,p_expires_at timestamptz
)
returns jsonb language sql security invoker set search_path=''
as $$ select private.copilot_register_followup_create_proposal_v1_impl(
  p_workspace_id,p_actor_user_id,p_request_id,p_proposal_hash,p_transaction_id,p_followup_id,p_title,p_due_at,p_expires_at
); $$;

create or replace function public.copilot_execute_followup_create_v1(
  p_workspace_id uuid,p_proposal_id uuid,p_proposal_hash text,p_execution_key uuid
)
returns jsonb language sql volatile security invoker set search_path=''
as $$ select private.copilot_execute_followup_create_v1_impl(
  p_workspace_id,p_proposal_id,p_proposal_hash,p_execution_key
); $$;

revoke all on function private.copilot_begin_request_v5_impl(uuid,uuid,uuid,text,text,integer)
  from public,anon,authenticated,service_role;
grant execute on function private.copilot_begin_request_v5_impl(uuid,uuid,uuid,text,text,integer)
  to service_role;

revoke all on function public.copilot_begin_request_v5(uuid,uuid,uuid,text,text,integer)
  from public,anon,authenticated;
grant execute on function public.copilot_begin_request_v5(uuid,uuid,uuid,text,text,integer)
  to service_role;

revoke all on function private.copilot_register_followup_create_proposal_v1_impl(uuid,uuid,uuid,text,uuid,uuid,text,timestamptz,timestamptz)
  from public,anon,authenticated,service_role;
grant execute on function private.copilot_register_followup_create_proposal_v1_impl(uuid,uuid,uuid,text,uuid,uuid,text,timestamptz,timestamptz)
  to service_role;

revoke all on function public.copilot_register_followup_create_proposal_v1(uuid,uuid,uuid,text,uuid,uuid,text,timestamptz,timestamptz)
  from public,anon,authenticated;
grant execute on function public.copilot_register_followup_create_proposal_v1(uuid,uuid,uuid,text,uuid,uuid,text,timestamptz,timestamptz)
  to service_role;

revoke all on function private.copilot_execute_followup_create_v1_impl(uuid,uuid,text,uuid)
  from public,anon,service_role;
grant execute on function private.copilot_execute_followup_create_v1_impl(uuid,uuid,text,uuid)
  to authenticated;

revoke all on function public.copilot_execute_followup_create_v1(uuid,uuid,text,uuid)
  from public,anon,service_role;
grant execute on function public.copilot_execute_followup_create_v1(uuid,uuid,text,uuid)
  to authenticated;
