-- ENJAZ Phase 11.6-B — authenticated Real Cloud intake follow-up probe
-- Requires the two Phase 11.6-B authority migrations. Fixtures are deleted before commit.
begin;
...
('11620000-0000-4000-8000-000000000202','11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000101','P116B unrelated','active','normal',1);
...
('11620000-0000-4000-8000-000000000301','11620000-0000-4000-8000-000000000001','__P116B_BOUND_LEAD__','probe','converted','11620000-0000-4000-8000-000000000101','11620000-0000-4000-8000-000000000201',current_setting('p116b.actor')::uuid),
...
('11620000-0000-4000-8000-000000000402','11620000-0000-4000-8000-000000000002','__P116B_FORM_B__','اختبار العزل','اختبار حي مؤقت',true,current_setting('p116b.actor')::uuid);
...
-- Staff issue path and destructive authority checks.
...
begin
  ...
  begin
    insert into private.intake_followup_requests(workspace_id,submission_id,mode,request_kind,title,requested_fields,token_hash,source_submission_version,expected_submission_version,expires_at,idempotency_key,requested_by)
    values('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000601','secure_link','information','FORBIDDEN','["phone"]',repeat('f',64),1,1,now()+interval '1 hour','11620000-0000-4000-8000-000000009999',current_setting('p116b.actor')::uuid);
    raise exception 'P116B_PRIVATE_DIRECT_INSERT_NOT_BLOCKED';
  exception when insufficient_privilege then null; end;

  begin
    perform public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000607',1,'secure_link','information','["phone"]','Cross workspace',null,24,'11620000-0000-4000-8000-000000000801',null,null,null);
    raise exception 'P116B_CROSS_WORKSPACE_NOT_REJECTED';
  exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_SUBMISSION_NOT_FOUND%' then raise; end if; end;

  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000601',1,'secure_link','information','["phone","email"]','استكمال البيانات','يرجى التحديث',24,'11620000-0000-4000-8000-000000000802',null,null,null);
  ...
  perform set_config('p116b.secure_token',v->>'token',true);
  perform set_config('p116b.secure_followup',v->>'followupId',true);

  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000601',1,'secure_link','information','["phone","email"]','استكمال البيانات','يرجى التحديث',24,'11620000-0000-4000-8000-000000000802',null,null,null);
  if coalesce((v->>'wasDuplicate')::boolean,false) is not true or v->>'token'<>current_setting('p116b.secure_token') then raise exception 'P116B_IDEMPOTENT_RETRY_FAILED %',v; end if;

  begin
    perform public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000601',1,'secure_link','information','["phone","email"]','CHANGED',null,24,'11620000-0000-4000-8000-000000000802',null,null,null);
    raise exception 'P116B_IDEMPOTENCY_CONFLICT_NOT_REJECTED';
  exception when unique_violation then null; end;

  begin
    perform public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000601',2,'secure_link','information','["phone"]','Second open',null,24,'11620000-0000-4000-8000-000000000803',null,null,null);
    raise exception 'P116B_SECOND_OPEN_NOT_REJECTED';
  exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_ALREADY_OPEN%' then raise; end if; end;

  begin
    perform public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000606',1,'client_portal','information','["email"]','Unbound portal',null,24,'11620000-0000-4000-8000-000000000804','11620000-0000-4000-8000-000000000701','11620000-0000-4000-8000-000000000201','11620000-0000-4000-8000-000000000901');
    raise exception 'P116B_UNBOUND_PORTAL_NOT_REJECTED';
  exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_PORTAL_TRANSACTION_UNBOUND%' then raise; end if; end;
end $$;
reset role;

-- Token is hashed only; canonical submission entered review once.
do $$
declare v_hash text;
begin
  select token_hash into v_hash from private.intake_followup_requests where id=current_setting('p116b.secure_followup')::uuid;
  if v_hash<>encode(extensions.digest(convert_to(current_setting('p116b.secure_token'),'UTF8'),'sha256'),'hex') then raise exception 'P116B_TOKEN_HASH_INVALID'; end if;
  ...
  if (select status from public.intake_submissions where id='11620000-0000-4000-8000-000000000601')<>'under_review' or (select version from public.intake_submissions where id='11620000-0000-4000-8000-000000000601')<>2 then raise exception 'P116B_REVIEW_STATE_INVALID'; end if;
  perform set_config('p116b.answers_before',(select answers::text from public.intake_submissions where id='11620000-0000-4000-8000-000000000601'),true);
end $$;

-- Anonymous caller can use only the token capability; no direct table read is needed or granted.
set local role anon;
do $$
declare v jsonb;
begin
  ...
  begin
    perform public.save_public_intake_followup_v1(current_setting('p116b.secure_token'),'{"bad":"x"}',false);
    raise exception 'P116B_OUT_OF_SCOPE_PATCH_NOT_REJECTED';
  exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_PATCH_SCOPE_INVALID%' then raise; end if; end;
  v:=public.save_public_intake_followup_v1(current_setting('p116b.secure_token'),'{"phone":"07711111111"}',false);
  if v->>'status'<>'open' or (v->>'submissionVersion')::int<>2 then raise exception 'P116B_DRAFT_SAVE_INVALID %',v; end if;
end $$;
reset role;

-- Draft did not mutate intake truth.
do $$
begin
  if (select answers::text from public.intake_submissions where id='11620000-0000-4000-8000-000000000601')<>current_setting('p116b.answers_before') then raise exception 'P116B_DRAFT_MUTATED_SUBMISSION'; end if;
end $$;
...
declare v jsonb;
begin
  v:=public.save_public_intake_followup_v1(current_setting('p116b.secure_token'),'{"phone":"07711111111","email":"probe@example.com"}',true);
  ...
  v:=public.save_public_intake_followup_v1(current_setting('p116b.secure_token'),'{"phone":"07711111111","email":"probe@example.com"}',true);
  if coalesce((v->>'wasDuplicate')::boolean,false) is not true or (v->>'submissionVersion')::int<>3 then raise exception 'P116B_FINALIZE_REPLAY_INVALID %',v; end if;
end $$;
reset role;

do $$
begin
  if (select status from public.intake_submissions where id='11620000-0000-4000-8000-000000000601')<>'under_review' then raise exception 'P116B_FOLLOWUP_AUTO_APPROVED'; end if;
...
declare v jsonb;
begin
  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000602',1,'secure_link','information','["phone"]','Revoke probe',null,24,'11620000-0000-4000-8000-000000000805',null,null,null);
  perform set_config('p116b.revoke_token',v->>'token',true); perform set_config('p116b.revoke_followup',v->>'followupId',true);
  ...
  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000603',1,'secure_link','information','["phone"]','Expiry probe',null,24,'11620000-0000-4000-8000-000000000806',null,null,null);
  perform set_config('p116b.expire_token',v->>'token',true); perform set_config('p116b.expire_followup',v->>'followupId',true);
end $$;
reset role;
...
do $$
begin
  begin perform public.get_public_intake_followup_v1(current_setting('p116b.revoke_token')); raise exception 'P116B_REVOKED_TOKEN_NOT_REJECTED'; exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_REVOKED%' then raise; end if; end;
  begin perform public.get_public_intake_followup_v1(current_setting('p116b.expire_token')); raise exception 'P116B_EXPIRED_TOKEN_NOT_REJECTED'; exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_EXPIRED%' then raise; end if; end;
end $$;
reset role;

-- Portal information path.
select set_config('request.jwt.claims',jsonb_build_object('role','authenticated','sub',current_setting('p116b.actor'))::text,true);
select set_config('request.jwt.claim.sub',current_setting('p116b.actor'),true);
set local role authenticated;
do $$
declare v jsonb;
begin
  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000604',1,'client_portal','information','["email"]','Portal info','أرسل البريد الصحيح',24,'11620000-0000-4000-8000-000000000807','11620000-0000-4000-8000-000000000701','11620000-0000-4000-8000-000000000201','11620000-0000-4000-8000-000000000901');
  ...
  begin
    perform public.reconcile_portal_intake_followup_v1('11620000-0000-4000-8000-000000000001',(v->>'followupId')::uuid,1,2,'{"email":"client@example.com"}');
    raise exception 'P116B_RECONCILE_WITHOUT_CLIENT_EVIDENCE';
  exception when others then if sqlerrm not like '%ENJAZ_INTAKE_FOLLOWUP_PORTAL_REQUEST_NOT_FULFILLED%' then raise; end if; end;
end $$;
reset role;
...
begin
  ...
  v:=public.send_client_portal_message_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000201','11620000-0000-4000-8000-000000000901','11620000-0000-4000-8000-000000000911','client@example.com');
...
declare v jsonb;
begin
  v:=public.reconcile_portal_intake_followup_v1('11620000-0000-4000-8000-000000000001',current_setting('p116b.portal_followup')::uuid,1,2,'{"email":"client@example.com"}');
  if v->>'status'<>'responded' or (v->>'submissionVersion')::int<>3 then raise exception 'P116B_PORTAL_RECONCILE_FAILED %',v; end if;

  v:=public.issue_intake_followup_v1('11620000-0000-4000-8000-000000000001','11620000-0000-4000-8000-000000000605',1,'client_portal','document','[]','Portal document','ارفع المستند',24,'11620000-0000-4000-8000-000000000808','11620000-0000-4000-8000-000000000701','11620000-0000-4000-8000-000000000201','11620000-0000-4000-8000-000000000902');
  if v->>'requestKind'<>'document' then raise exception 'P116B_DOCUMENT_DELEGATION_INVALID %',v; end if;
  perform set_config('p116b.document_followup',v->>'followupId',true);
  v:=public.revoke_intake_followup_v1('11620000-0000-4000-8000-000000000001',(v->>'followupId')::uuid,1,'probe cleanup revocation');
  if v->>'status'<>'revoked' then raise exception 'P116B_PORTAL_REVOKE_FAILED %',v; end if;
end $$;
reset role;
...
begin
  if (select status from public.client_portal_requests where id='11620000-0000-4000-8000-000000000901')<>'fulfilled' then raise exception 'P116B_M3_INFO_REQUEST_NOT_FULFILLED'; end if;
  ...
  if (select request_type from public.client_portal_requests where id='11620000-0000-4000-8000-000000000902')<>'document' then raise exception 'P116B_DOCUMENT_NOT_DELEGATED_TO_M3'; end if;
  if (select revoked_at from public.client_portal_requests where id='11620000-0000-4000-8000-000000000902') is null then raise exception 'P116B_PORTAL_REVOKE_NOT_PROPAGATED'; end if;
  select count(*) into v_count from public.audit_events where workspace_id='11620000-0000-4000-8000-000000000001' and action in ('intake.followup.requested','intake.followup.responded','intake.followup.portal_reconciled','intake.followup.revoked');
  if v_count<7 then raise exception 'P116B_AUDIT_EVIDENCE_INCOMPLETE %',v_count; end if;
end $$;

-- Workspace cascade must remove every probe row; system HMAC secret intentionally remains.
...
begin
  ...
  if exists(select 1 from public.client_portal_requests where workspace_id::text like '11620000-0000-4000-8000-%') then raise exception 'P116B_PORTAL_RESIDUE'; end if;
