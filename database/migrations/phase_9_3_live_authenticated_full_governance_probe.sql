...
-- Evidence-only migration. No auth identities are created. All temporary domain fixtures are removed before commit.
...
create or replace function private.enjaz_phase93_m2_expect_owner_required(p_workspace_id uuid,p_company_id uuid,p_contact_id uuid)
returns boolean language plpgsql security invoker set search_path='' as $$
begin
...
create or replace function private.enjaz_phase93_m2_expect_read_denied(p_workspace_id uuid,p_company_id uuid,p_expected text)
returns boolean language plpgsql security invoker set search_path='' as $$
begin
  perform public.get_company_governance_context_v1(p_workspace_id,p_company_id,date '2026-02-01');
  return false;
exception when insufficient_privilege then return sqlerrm=p_expected;
end;
$$;

create or replace function private.enjaz_phase93_m2_expect_cross_contact_denied(p_workspace_id uuid,p_company_id uuid,p_contact_id uuid)
returns boolean language plpgsql security invoker set search_path='' as $$
begin
  perform public.replace_company_beneficial_owners_v1(
    p_workspace_id,p_company_id,1,gen_random_uuid(),date '2026-04-01',
...
revoke all on function private.enjaz_phase93_m2_expect_owner_required(uuid,uuid,uuid) from public,anon;
revoke all on function private.enjaz_phase93_m2_expect_read_denied(uuid,uuid,text) from public;
revoke all on function private.enjaz_phase93_m2_expect_cross_contact_denied(uuid,uuid,uuid) from public,anon;
...
grant execute on function private.enjaz_phase93_m2_expect_owner_required(uuid,uuid,uuid) to authenticated;
grant execute on function private.enjaz_phase93_m2_expect_read_denied(uuid,uuid,text) to authenticated,anon;
grant execute on function private.enjaz_phase93_m2_expect_cross_contact_denied(uuid,uuid,uuid) to authenticated;
...
select private.enjaz_phase93_m2_probe_assert(
  not exists(select 1 from public.workspaces where id='93f00000-0000-4000-8000-000000000002'::uuid)
...
values('93f00000-0000-4000-8000-000000000002'::uuid,current_setting('p93m2.owner_user')::uuid,'__ENJAZ_PHASE93_M2_FOREIGN_WS__');
...
('93f00000-0000-4000-8000-000000000013'::uuid,'93f00000-0000-4000-8000-000000000002'::uuid,'M2 Foreign Person','client','active');
...
with x as (select public.replace_company_beneficial_owners_v1(
  current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,0,current_setting('p93m2.bo_op')::uuid,date '2026-01-01',
  jsonb_build_array(jsonb_build_object('contactId','93f00000-0000-4000-8000-000000000011','basis','ownership','percentage','100'))
) body) select private.enjaz_phase93_m2_probe_assert((body->>'version')::int=1 and body->>'replayed'='true','beneficial-owner replay failed') from x;

select set_config('p93m2.grant_id',(
  public.grant_company_authority_v1(
    current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,0,gen_random_uuid(),
...
select private.enjaz_phase93_m2_probe_assert(private.enjaz_phase93_m2_expect_cross_contact_denied(current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,'93f00000-0000-4000-8000-000000000013'::uuid),'cross-workspace beneficial owner was accepted');
...
-- A non-owner authenticated JWT (deliberately absent from domain memberships) must fail closed.
select set_config('request.jwt.claims',jsonb_build_object('role','authenticated','sub','93f00000-0000-4000-8000-000000000099')::text,true);
select set_config('request.jwt.claim.sub','93f00000-0000-4000-8000-000000000099',true);
select private.enjaz_phase93_m2_probe_assert(private.enjaz_phase93_m2_expect_owner_required(current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,'93f00000-0000-4000-8000-000000000011'::uuid),'non-owner governance mutation was accepted');
select private.enjaz_phase93_m2_probe_assert(private.enjaz_phase93_m2_expect_read_denied(current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,'ENJAZ_ORG_WORKSPACE_FORBIDDEN'),'foreign authenticated read was accepted');

reset role;
set local role anon;
select set_config('request.jwt.claims',jsonb_build_object('role','anon')::text,true);
select set_config('request.jwt.claim.sub','',true);
select private.enjaz_phase93_m2_probe_assert(private.enjaz_phase93_m2_expect_read_denied(current_setting('p93m2.owner_ws')::uuid,'93f00000-0000-4000-8000-000000000010'::uuid,'ENJAZ_ORG_AUTH_REQUIRED'),'anonymous governance read was accepted');
...
-- Zero-residue cleanup. The temporary foreign workspace uses the existing owner identity only.
...
delete from public.contacts where id='93f00000-0000-4000-8000-000000000013'::uuid;
delete from public.workspaces where id='93f00000-0000-4000-8000-000000000002'::uuid;
...
drop function private.enjaz_phase93_m2_expect_cross_contact_denied(uuid,uuid,uuid);
drop function private.enjaz_phase93_m2_expect_read_denied(uuid,uuid,text);
drop function private.enjaz_phase93_m2_expect_owner_required(uuid,uuid,uuid);
