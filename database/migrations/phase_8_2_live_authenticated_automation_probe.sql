...
-- Exercises the exact public RPC boundary as a real existing authenticated user,
-- proves replay/stale/approval behavior, then removes every probe row before commit.
begin;
...
select private.enjaz_phase82_probe_assert(
  ...
  'no authenticated workspace member exists'
);
...
select private.enjaz_phase82_probe_assert(
  nullif(current_setting('enjaz.probe_workspace_id', true), '') is not null,
...
with inserted as (
  ...
  values (
    ...
    '__ENJAZ_PHASE82_PROBE_COMPANY__',
...
select private.enjaz_phase82_probe_assert(
  ...
  'authenticated regained direct automation_rules mutation privileges'
);
select private.enjaz_phase82_probe_assert(
  not has_table_privilege('public.automation_runs', 'INSERT')
  ...
  'authenticated regained direct automation execution-table mutation privileges'
);
...
select private.enjaz_phase82_probe_assert(
  nullif(current_setting('enjaz.probe_followup_rule_id', true), '') is not null,
  'authenticated upsert RPC could not create a rule'
);

select set_config('enjaz.probe_stale_guard', 'missed', true);
do $$
begin
  begin
    perform public.set_automation_rule_enabled_v1(
      current_setting('enjaz.probe_workspace_id')::uuid,
  ...
  exception
    when serialization_failure then
      if sqlerrm = 'ENJAZ_AUTOMATION_RULE_STALE' then
        perform set_config('enjaz.probe_stale_guard', 'hit', true);
      else
        raise;
      end if;
  end;
...
select private.enjaz_phase82_probe_assert(
  current_setting('enjaz.probe_stale_guard', true) = 'hit',
  'stale-version protection did not fail closed'
);
...
select
  ...
  private.enjaz_phase82_probe_assert(body->>'status' = 'succeeded', 'followup dispatch did not succeed'),
  private.enjaz_phase82_probe_assert(not (body->>'wasDuplicate')::boolean, 'first followup dispatch was incorrectly marked duplicate')
from result;
...
select private.enjaz_phase82_probe_assert(
  ...
  'idempotent followup dispatch produced zero or duplicate side effects'
);
...
select
  ...
  private.enjaz_phase82_probe_assert(body->>'status' = 'awaiting_approval', 'sensitive workflow action bypassed human approval'),
  private.enjaz_phase82_probe_assert(not (body->>'wasDuplicate')::boolean, 'first approval dispatch was incorrectly marked duplicate')
from result;
...
select private.enjaz_phase82_probe_assert(
  ...
  'pending human approval evidence was not created'
);
...
select private.enjaz_phase82_probe_assert(
  body->>'runStatus' = 'skipped'
  and not (body->>'wasDuplicate')::boolean,
  'human rejection did not skip the guarded run'
) from decision;
...
select private.enjaz_phase82_probe_assert(
  (body->>'wasDuplicate')::boolean
  and body->>'runStatus' = 'skipped',
  'approval decision replay was not idempotent'
) from replay;

with context as (
  select public.get_automation_engine_context_v1(
    current_setting('enjaz.probe_workspace_id')::uuid
  ) as body
)
select private.enjaz_phase82_probe_assert(
  ...
  'authenticated automation context authority drifted'
) from context;
