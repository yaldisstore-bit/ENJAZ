...
-- Runs against the live project inside one transaction, simulates real authenticated JWTs,
-- verifies owner/workforce/RLS/downward inheritance/ownership isolation, then ROLLBACKs all probe data.
...
select set_config('enjaz.p85.owner_user',(
  ...
  where wm.role='owner' order by wm.created_at,wm.workspace_id limit 1
),true);
select private.enjaz_phase85_probe_assert(nullif(current_setting('enjaz.p85.owner_user',true),'') is not null,'no real owner exists');
...
select private.enjaz_phase85_probe_assert(
  ...
  'fixed probe identities already exist'
);
...
values(
  '85000000-0000-4000-8000-000000000002'::uuid,current_setting('enjaz.p85.owner_ws')::uuid,
...
values(
  '85000000-0000-4000-8000-000000000003'::uuid,current_setting('enjaz.p85.owner_ws')::uuid,
  '85000000-0000-4000-8000-000000000002'::uuid,'__ENJAZ_PHASE85_PROBE_TRANSACTION__','QA','active','normal',100.00
);
select set_config('enjaz.p85.tx_updated_before',(
  select updated_at::text from public.transactions where id='85000000-0000-4000-8000-000000000003'::uuid
),true);
...
select private.enjaz_phase85_probe_assert(
  ...
  'authenticated direct organization privileges are not read-only'
);
select private.enjaz_phase85_probe_assert(
...
select private.enjaz_phase85_probe_assert(
  ...
  'owner organization authority context drifted'
) from c;
...
with x as (select public.set_organization_member_v1(
  current_setting('enjaz.p85.owner_ws')::uuid,current_setting('enjaz.p85.workforce_user')::uuid,'active',null
) body)
select set_config('enjaz.p85.member_id',body->>'memberId',true) from x;
select private.enjaz_phase85_probe_assert(
  not exists(select 1 from public.workspace_memberships wm where wm.workspace_id=current_setting('enjaz.p85.owner_ws')::uuid and wm.user_id=current_setting('enjaz.p85.workforce_user')::uuid),
  'workforce leaked into legacy workspace_memberships'
);
with x as (select public.set_organization_scope_membership_v1(
  current_setting('enjaz.p85.owner_ws')::uuid,null,null,current_setting('enjaz.p85.workforce_user')::uuid,
  'branch',current_setting('enjaz.p85.branch_a')::uuid,null,null,'manager','active',null,null
) body)
...
with x as (select public.assign_transaction_organization_v1(
  current_setting('enjaz.p85.owner_ws')::uuid,'85000000-0000-4000-8000-000000000003'::uuid,null,
  'branch',current_setting('enjaz.p85.branch_a')::uuid,null,null,'Phase 8.5 initial owner assignment'
) body)
select set_config('enjaz.p85.ownership_id',body->>'ownershipId',true),set_config('enjaz.p85.ownership_version',body->>'version',true) from x;
select private.enjaz_phase85_probe_assert(
  current_setting('enjaz.p85.ownership_version')::integer=1
  and (select updated_at::text=current_setting('enjaz.p85.tx_updated_before') from public.transactions where id='85000000-0000-4000-8000-000000000003'::uuid)
  ...
  'organizational assignment mutated transaction lifecycle or finance'
);

-- Scoped workforce session: explicit branch membership must flow only downward.
reset role;
...
select private.enjaz_phase85_probe_assert(
  not exists(select 1 from public.workspace_memberships wm where wm.workspace_id=current_setting('enjaz.p85.owner_ws')::uuid and wm.user_id=auth.uid()),
...
select private.enjaz_phase85_probe_assert(
  ...
  'RLS did not enforce branch downward inheritance and sibling isolation'
);
...
select private.enjaz_phase85_probe_assert(
  ...
  'workforce context leaked siblings or lost explicit inheritance source'
) from c;
...
select private.enjaz_phase85_probe_assert(
  ...
  'downward inherited access explanation is not source-explainable'
) from e;
with e as (select public.explain_organization_access_v1(
...
select private.enjaz_phase85_probe_assert(
  private.enjaz_phase85_expect_transfer_denied(
    current_setting('enjaz.p85.owner_ws')::uuid,'85000000-0000-4000-8000-000000000003'::uuid,1,current_setting('enjaz.p85.branch_b')::uuid
  ),
  'workforce could transfer ownership into an unmanaged sibling branch'
);
with x as (select public.assign_transaction_organization_v1(
  current_setting('enjaz.p85.owner_ws')::uuid,'85000000-0000-4000-8000-000000000003'::uuid,1,
  'team',null,null,current_setting('enjaz.p85.team_a')::uuid,'Phase 8.5 manager downward transfer'
) body)
select private.enjaz_phase85_probe_assert(
  (body->>'version')::integer=2 and body->>'scopeType'='team' and body->>'teamId'=current_setting('enjaz.p85.team_a'),
  'branch manager could not transfer to managed descendant team'
) from x;
select private.enjaz_phase85_probe_assert(
  ...
  and not exists(select 1 from public.payments where transaction_id='85000000-0000-4000-8000-000000000003'::uuid)
  and not exists(select 1 from public.financial_ledger_entries where transaction_id='85000000-0000-4000-8000-000000000003'::uuid),
  'workforce ownership transfer mutated transaction lifecycle or finance'
);

-- Cross-workspace fail-closed: the real owner must not inherit access to the temporary user's workspace.
reset role;
select set_config('request.jwt.claims',jsonb_build_object('role','authenticated','sub',current_setting('enjaz.p85.owner_user'))::text,true);
select set_config('request.jwt.claim.sub',current_setting('enjaz.p85.owner_user'),true);
set local role authenticated;
...
select private.enjaz_phase85_probe_assert(
  ...
  'ownership audit trail did not record assigned + transferred events'
);

rollback;

-- Independent zero-residue proof after rollback.
select jsonb_build_object(
  'status','PASS',
  'authUserResidue',(select count(*) from auth.users where id='85000000-0000-4000-8000-000000000001'::uuid or email='__enjaz_phase85_probe_workforce__@example.invalid'),
  'companyResidue',(select count(*) from public.companies where id='85000000-0000-4000-8000-000000000002'::uuid or legal_name='__ENJAZ_PHASE85_PROBE_COMPANY__'),
  'transactionResidue',(select count(*) from public.transactions where id='85000000-0000-4000-8000-000000000003'::uuid or type='__ENJAZ_PHASE85_PROBE_TRANSACTION__'),
  'memberResidue',(select count(*) from public.organization_members where user_id='85000000-0000-4000-8000-000000000001'::uuid),
  'branchResidue',(select count(*) from public.organization_branches where name like '__ENJAZ_PHASE85_PROBE_BRANCH_%'),
  'departmentResidue',(select count(*) from public.organization_departments where name like '__ENJAZ_PHASE85_PROBE_DEPT_%'),
  'teamResidue',(select count(*) from public.organization_teams where name like '__ENJAZ_PHASE85_PROBE_TEAM_%'),
  'ownershipResidue',(select count(*) from public.transaction_organization_ownership where transaction_id='85000000-0000-4000-8000-000000000003'::uuid),
  'ownershipEventResidue',(select count(*) from public.transaction_organization_ownership_events where transaction_id='85000000-0000-4000-8000-000000000003'::uuid)
) as phase85_real_cloud_probe;
