...
const scenarios = [
  ...
  { name: 'downgrade application phase version', file: 'src/core/version/version.ts', mutate: (value) => value.replace(/APP_VERSION = '[^']+'/, "APP_VERSION = '0.9.0-phase2.7'") },
  { name: 'downgrade package phase version', file: 'package.json', mutate: (value) => value.replace(/"version": "[^"]+"/, '"version": "0.9.0-phase2.7"') },
  { name: 'downgrade workflow below destruction gate', file: '.github/workflows/enjaz-quality-gate.yml', mutate: (value) => value.replace(/Full Phase \d+\.\d+ verification/, 'Full Phase 2.7 verification').replace(/npm run verify:phase\d+\.\d+/, 'npm run verify:phase2.7') },
