...
const scenarios = [
  ...
  { name: 'downgrade application version below Phase 3.2', file: 'src/core/version/version.ts', mutate: (value) => value.replace(/APP_VERSION = '[^']+'/, "APP_VERSION = '0.10.0-phase3.1'") },
  { name: 'downgrade package version below Phase 3.2', file: 'package.json', mutate: (value) => value.replace(/"version": "[^"]+"/, '"version": "0.10.0-phase3.1"') },
  ...
  { name: 'downgrade GitHub verification below Phase 3.2', file: '.github/workflows/enjaz-quality-gate.yml', mutate: (value) => value.replace(/Full Phase \d+\.\d+ verification/g, 'Full Phase 3.1 verification').replace(/npm run verify:phase\d+\.\d+/g, 'npm run verify:phase3.1') },
