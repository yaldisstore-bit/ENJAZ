...
const hardening=read('database/migrations/phase_10_2_stale_source_hardening.sql');
...
for(const marker of ['supersede_document_intelligence_on_new_version_v1','document_versions_supersede_intelligence_v1',"failure_code='SOURCE_VERSION_STALE'","verification_state='superseded'","v_stale:=not found or v.document_version_id is null or v_latest.id<>v.document_version_id","'verified',false","'SOURCE_VERSION_CHANGED'","'promotedToSource',false"])check(`hardening:${marker}`,has(hardening,marker));
check('hardening_never_rewrites_source',!/(update|delete\s+from)\s+public\.(documents|document_versions)\b/i.test(hardening));
...
for(const marker of ['1280','390','320','derived-even-when-verified','اعتماد المراجعة','تحقق نهائي من النتيجة','contract-v2.pdf','data-analysis-stale',"data-analysis-state','superseded",'versionNumber).toBe(2)','scrollWidth'])check(`browser_spec:${marker}`,has(browserSpec,marker));
...
for(const marker of ['direct_browser_analysis_insert_denied','verified_derived_not_source','new_source_version_supersedes_verified_intelligence','stale_reverify_cannot_restore_verification','provider_absence_fail_safe_explicit','provider_absence_persisted_as_explicit_failure','real_cloud_document_intelligence_boundary_passed'])check(`real_cloud:${marker}`,has(realCloud,marker));
check('real_cloud_zero_residue_cleanup',has(realCloud,"admin.storage.from(BUCKET).remove")&&has(realCloud,"admin.from('workspaces').delete()")&&has(realCloud,'admin.auth.admin.deleteUser'));
