...
if (!errors.length) {
  ...
  if (state.applicationShell?.status !== 'CLOSED' || state.applicationShell?.exitGatePassed !== true) {
    errors.push('applicationShell state must be CLOSED with exitGatePassed=true');
  }
  if (state.applicationShell?.canonicalRuntimeChanged !== false) errors.push('R2.0-3 may not change canonical runtime');
  if (state.goldenExperience?.implemented !== false || state.goldenExperience?.userApproved !== false) {
    errors.push('R2.0-3 must not claim Golden Experience implementation or approval');
  }

  if (manifest.stage !== 'R2.0-3' || manifest.status !== 'FROZEN' || manifest.exitGatePassed !== true) {
    errors.push('shell manifest must be frozen for R2.0-3 with exitGatePassed=true');
  }
  ...
  if (!manifest.historyFoundations?.pushState || !manifest.historyFoundations?.popState || !manifest.historyFoundations?.deepLinkQueryState) {
    errors.push('shell history/deep-link foundations are incomplete');
  }
  ...
  for (const width of [1280, 430, 390, 360, 320]) {
    if (!manifest.responsive?.hardWidths?.includes(width)) errors.push(`shell manifest missing hard viewport ${width}`);
  }
  ...
  for (const label of ['الرئيسية', 'المعاملات', 'جديد', 'اليوم', 'المزيد']) {
    if (!read('src/ui-r2/architecture/navigation-contract.ts').includes(label)) errors.push(`navigation contract missing primary label: ${label}`);
  }
  ...
  if (!vite.includes("r2Preview: 'r2-preview.html'")) errors.push('Vite must build the isolated r2-preview.html entrypoint');
  ...
  if (/ui-v2|ui-rebirth/.test(shell) || /ui-v2|ui-rebirth/.test(css)) {
    errors.push('R2.0-3 shell may not import or reference old presentation generations');
  }
}
...
if (errors.length) {
  ...
  console.log('ENJAZ REBIRTH 2.0 R2.0-3 APPLICATION SHELL AUDIT PASS — isolated UiR2Root, five-door shell, location/search/overlay/history foundations, mobile+desktop composition, canonical ui-v2 preserved.');
}
