...
requireValue(phase94.phase === '9.4' && phase94.status === 'IN_PROGRESS', 'Phase 9.4 must be IN_PROGRESS');
...
requireValue(phase94.phase9_5Allowed === false && phase94.exitGatePassed === false && phase94.successorStatus === 'LOCKED', 'Phase 9.5 must remain locked');
...
requireValue(m8?.closureEvidence === null, 'M8 cannot have global closure evidence at foundation kickoff');
...
]) requireValue(exists(path), `missing Phase 9.4 foundation artifact: ${path}`);
...
for (let index = 1; index <= 12; index += 1) {
  requireMarker(tests, `9.4 foundation ${String(index).padStart(2, '0')}`, 'foundation tests');
}
requireMarker(kickoff, 'AI summaries are derived artifacts only', 'kickoff');
requireMarker(kickoff, 'Phase 9.5 remains **LOCKED**', 'kickoff');
