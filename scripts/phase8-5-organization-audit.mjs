...
if(state.delivery?.phaseSliceJourney!=='isolated_live_preview_real_chromium'||state.delivery?.canonicalRuntimePromotion!=='DEFERRED_UNTIL_SEPARATE_BUDGETED_PROMOTION'||state.delivery?.canonicalRuntimeMustRemainAtPredecessorBudgetContract!==true||state.delivery?.duplicateNavigationDestinationAllowed!==false)throw new Error('Phase 8.5 isolated delivery/budget contract drift');
...
for(const marker of ['features/organization/','../organization/organization.css','OrganizationCommandProvider','OrganizationWorkspaceResolverProvider','OrganizationAssignmentCatalogProvider']){
  if(production.includes(marker))throw new Error(`Canonical production runtime must not import Phase 8.5 live slice before separate budgeted promotion: ${marker}`);
}
for(const marker of ['LiveOrganizationExperience','data-account-workspace-home="8.5"']){
  if(liveRoot.includes(marker))throw new Error(`Canonical live root must remain predecessor-identical during isolated Phase 8.5 slice: ${marker}`);
}
...
console.log('ENJAZ PHASE 8.5 ORGANIZATION AUDIT PASS — legacy owner membership untouched; workforce authorization isolated from legacy RLS/RPCs; branch→department→team inheritance is downward/source-explainable; direct org DML denied; isolated M15 UI/Chromium journey is kept out of the canonical bundle under the unchanged 670000-byte contract; frozen IA remains unchanged; transaction ownership is audited without lifecycle/finance writes; Phase 8.6 remains LOCKED.');
