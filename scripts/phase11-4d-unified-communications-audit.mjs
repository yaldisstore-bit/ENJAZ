...
for(const marker of [
  'PHASE114D_HUB_CONVERSATIONS_EMPTY','PHASE114D_TIMELINE_COUNT_FAIL','PHASE114D_UNREAD_NOT_DERIVED',
  'PHASE114D_AWAITING_STAFF_FAIL','PHASE114D_SEARCH_FAIL','PHASE114D_READ_CURSOR_FAIL',
  'PHASE114D_DIRECT_READ_TABLE_ALLOWED','PHASE114D_SAFE_RETRY_FAIL','PHASE114D_RECONCILIATION_BLIND_RETRY_ALLOWED',
]) has(probe,marker,`Real Cloud destruction probe marker missing: ${marker}`);
...
for(const marker of ['get_communications_hub_v1','mark_communication_conversation_read_v1','retry_communication_outbound_v1','relink_communication_v1'])
  has(service,marker,`service must use governed RPC ${marker}`);
...
for(const marker of [
  'data-phase11-4-unified-communications="live"','data-communications-timeline="canonical"','data-communications-review="governed"',
  'بحث الاتصالات','إعادة آمنة','اختر محادثة أولًا','تحتاج مطابقة',
]) has(ui,marker,`unified communications UI marker missing: ${marker}`);
...
for(const breakpoint of ['@media (max-width:60rem)','@media (max-width:42rem)','@media (max-width:24rem)']) has(css,breakpoint,`responsive communications CSS missing ${breakpoint}`);

if(fail.length){
  console.error(`ENJAZ PHASE 11.4-D UNIFIED COMMUNICATIONS AUDIT FAIL (${fail.length})`);
  for(const item of fail) console.error(`- ${item}`);
  process.exit(1);
}
console.log('ENJAZ PHASE 11.4-D UNIFIED COMMUNICATIONS AUDIT PASS — canonical timeline, per-user unread, SLA, governed search/relink, safe retry, C lineage and M10 lock preserved.');
