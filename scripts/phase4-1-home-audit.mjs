...
const canonicalPhase42StatusLines = phase42StatusLines.filter((line) => /^\s*-\s+\*\*Phase 4\.2 — Daily Work \/ Universal Inbox\*\*\s+⏳\s+not started\s*$/.test(line));
check(
  'README keeps 4.2 not started',
  canonicalPhase42StatusLines.length === 1
