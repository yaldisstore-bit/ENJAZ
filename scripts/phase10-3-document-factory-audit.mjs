...
const cascadeHardening=read('database/migrations/phase_10_3_workspace_cascade_hardening.sql');
const submissionPack=read('database/migrations/phase_10_3_submission_pack_authority.sql');
...
const expansionTests=read('tests/documentFactoryExpansion.test.ts');
...
for(const marker of ["tg_op='DELETE' and old.status='published'","exists(select 1 from public.workspaces w where w.id=old.workspace_id)",'ENJAZ_TEMPLATE_VERSION_PUBLISHED_IMMUTABLE',"if old.status='final' then",'ENJAZ_DOCUMENT_FACTORY_FINAL_ARTIFACT_IMMUTABLE'])check(`cascade_hardening:${marker}`,has(cascadeHardening,marker));
check('cascade_hardening_never_disables_triggers',!/disable\s+trigger/i.test(cascadeHardening));

for(const marker of ['validate_document_factory_template_v1','save_document_template_v1','drop policy if exists document_templates_insert_workspace','revoke insert,update,delete on table public.document_templates','drop policy if exists document_drafts_direct_insert_workspace','revoke insert,update,delete on table public.document_drafts','generate_document_draft_v1','assert_document_factory_provenance_current_v1','select * into v_company from public.companies','select * into v_tx from public.transactions','select * into v_contact from public.contacts','select * into v_analysis from public.document_analysis','ENJAZ_DOCUMENT_FACTORY_OCR_NOT_VERIFIED','ENJAZ_DOCUMENT_FACTORY_OCR_STALE','update_document_draft_content_v1','submit_document_draft_for_review_v1','review_document_draft_v1','ENJAZ_DOCUMENT_FACTORY_DRAFT_CHECKSUM_DRIFT','get_document_factory_v1'])check(`runtime:${marker}`,has(runtime,marker));
...
for(const marker of ['create table if not exists public.document_submission_packs','create table if not exists public.document_submission_pack_items','enable row level security','revoke all on table public.document_submission_packs from public,anon,authenticated','request_document_submission_pack_v1','ENJAZ_SUBMISSION_PACK_REQUEST_DRIFT',"v_draft.status<>'final'",'v_draft.transaction_id is distinct from p_transaction_id',"lower(dv.mime_type)='application/pdf'",'mark_document_submission_pack_running_v1','complete_document_submission_pack_v1','fail_document_submission_pack_v1','to service_role'])check(`submission_pack:${marker}`,has(submissionPack,marker));
check('submission_pack_completion_not_browser_executable',!/grant execute on function public\.complete_document_submission_pack_v1[^\n]+authenticated/i.test(submissionPack));
check('submission_pack_tables_not_browser_mutable',!/grant\s+(insert|update|delete)[^;]+document_submission_packs[^;]+authenticated/i.test(submissionPack));
...
for(const marker of ['submission pack authority accepts finalized PDFs only and stays service-completed','renderer emits stable QR identity, conditional sections, repeated rows and transaction packs','browser gateway exposes governed template versions and pack build without server credentials','factory UI manages templates and only builds packs from final drafts on the same transaction'])check(`expansion_tests:${marker}`,has(expansionTests,marker));
for(const marker of ['scripts/phase10-3-document-factory-audit.mjs','tests/documentFactory.test.ts','tests/documentFactoryExpansion.test.ts','tests/documentFactoryFinalizationGuard.test.ts','tests/documentIntelligence.test.ts','tests/documentVault.test.ts','npm run test:functional','npm run db:audit','npm run audit:secrets','npm run typecheck','npm run build','Re-enforce Phase 10.3 certified state'])check(`workflow:${marker}`,has(workflow,marker));

for(const marker of ['createDocumentFactoryGateway','generate_document_draft_v1','review_document_draft_v1','request_document_render_v1','finalize_document_draft_v1','functions/v1/enjaz-document-render',".in('status',['queued','running','succeeded'])",'requested_by','listManagedTemplates','save_document_template_v1','create_document_template_version_v1','publish_document_template_version_v1','request_document_submission_pack_v1',"mode:'pack'",'get_document_submission_pack_v1'])check(`commands:${marker}`,has(commands,marker));
...
for(const marker of ["import QRCode from 'npm:qrcode@1.5.4'",'ENJAZ:DRAFT:','ENJAZ:PACK:','applyAdvancedLayout','\\[\\[IF\\s+','\\[\\[EACH\\s+','[[ITEM]]','drawTable','renderSubmissionPack','PACK_SOURCE_OBJECT_DRIFT',"p_document_type:'submission-pack'",'complete_document_submission_pack_v1'])check(`renderer_expansion:${marker}`,has(renderer,marker));
...
const packAuth=renderer.indexOf('auth.getUser()');
const packDispatch=renderer.indexOf("if(body.mode==='pack')return await handlePack(body,workspaceId,who.data.user.id,user,admin)");
check('pack_renderer_authenticated_dispatch',packAuth>=0&&packDispatch>packAuth&&has(renderer,'if(pack.requestedBy!==userId)'));
for(const marker of ['data-phase10-3="document-factory"','مصنع الوثائق الرسمية','Generate → Review → Render → Finalize','OCR متحقق','توليد مسودة رسمية','اعتماد المسودة','إصدار PDF واعتماد نهائي','فتح PDF النهائي','إدارة القوالب','حفظ + إنشاء نسخة ثابتة ونشرها','[[IF company.address]]','[[EACH ocr.items]]','إنشاء حزمة التقديم من جميع المستندات النهائية','نهائي · QR ثابت · محفوظ في Vault'])check(`factory_ui:${marker}`,has(factoryPanel,marker));
...
check('factory_ui_pack_is_final_same_transaction',has(factoryPanel,"d.status==='final'")&&has(factoryPanel,'d.transactionId===transactionId'));
