...
for(const marker of ["import QRCode from 'npm:qrcode@1.5.4'",'ENJAZ:DRAFT:','ENJAZ:PACK:','applyAdvancedLayout','[[IF','[[EACH','[[ITEM]]','drawTable','renderSubmissionPack','PACK_SOURCE_OBJECT_DRIFT',"p_document_type:'submission-pack'",'complete_document_submission_pack_v1'])check(`renderer_expansion:${marker}`,has(renderer,marker));
...
check('pack_renderer_user_auth_before_service',renderer.indexOf('auth.getUser()')>=0&&renderer.indexOf('auth.getUser()')<renderer.indexOf("admin.rpc('complete_document_submission_pack_v1'"));
