...
]) check(`pattern guard ${marker}`, contract.includes(marker));

for (const component of ['TransactionPattern', 'CompanyPattern', 'ContactPattern']) {
  check(`entity pattern exports ${component}`, entity.includes(`function ${component}`));
}
for (const component of ['FinanceSummaryPattern', 'RiskSignalPattern', 'TimelinePattern', 'FollowUpPattern', 'WorkflowPattern', 'AutomationPattern', 'CommandModulePattern']) {
  check(`operations pattern exports ${component}`, operations.includes(`function ${component}`));
}
for (const component of ['SearchResultPattern', 'ActionMenuPattern', 'SystemStatePattern', 'PatternSkeleton']) {
  check(`discovery pattern exports ${component}`, discovery.includes(`function ${component}`));
}
...
check('pattern CSS scopes hover to fine pointers', !/:hover/.test(allCss) || /@media \(hover: hover\) and \(pointer: fine\)/.test(allCss));
check('pattern CSS provides reduced motion handling', allCss.includes('@media (prefers-reduced-motion: reduce)'));
check('pattern CSS provides coarse pointer touch floor', patternCss.includes('@media (pointer: coarse)') && patternCss.includes('min-block-size: var(--size-touch-min)'));
check('pattern CSS provides normal mobile breakpoint', allCss.includes('@media (max-width: 48rem)'));
check('pattern CSS provides narrow phone breakpoint', allCss.includes('@media (max-width: 22.5rem)'));
...
for (const marker of [
  'TransactionPattern', 'CompanyPattern', 'ContactPattern', 'FinanceSummaryPattern', 'RiskSignalPattern', 'TimelinePattern',
  'FollowUpPattern', 'WorkflowPattern', 'AutomationPattern', 'CommandModulePattern', 'SearchResultPattern', 'ActionMenuPattern',
  'PatternSkeleton', 'density="compact"', '1250000000', 'longCompanyName',
...
for (const tone of ['empty', 'loading', 'success', 'warning', 'error', 'conflict', 'offline', 'recovery']) {
  check(`Pattern Lab proves ${tone} state`, lab.includes(`tone="${tone}"`));
}
...
for (const marker of ['Transaction', 'Company', 'Finance', 'Risk', 'Timeline', 'Follow-up', 'Workflow', 'Automation', 'Command Center', 'Search', 'System states', '/foundation/patterns', 'not complete product screens']) {
  check(`Phase 2.7 documentation contains ${marker}`, doc.includes(marker));
}
