...
async function assertUi9OrLater(page, label) {
  const app = page.locator('[data-core-app="true"]');
  await app.waitFor();
  const stage = await app.getAttribute('data-stage');
  const value = stage ? Number(stage.replace('ui-', '')) : 0;
  assert(value >= 9, `${label}: runtime regressed below UI-9 (${stage})`);
}
...
async function verifyStandardProfile(browser, profile) {
  ...
  await assertUi9OrLater(page, profile.name);
  ...
  const vv = await page.evaluate(() => ({ width:getComputedStyle(document.documentElement).getPropertyValue('--ez-visual-viewport-width'), height:getComputedStyle(document.documentElement).getPropertyValue('--ez-visual-viewport-height') }));
  ...
  await notifications.waitFor(); await page.waitForTimeout(240);
  ...
  await page.goBack();
  if (await account.count()) await page.locator('.ez-overlay[data-motion-state="closing"]').waitFor({state:'visible',timeout:1000});
  ...
  if (profile.mobile) {
    ...
    await create.waitFor(); await page.waitForTimeout(240);
    ...
    const keyboardDock=await page.locator('[data-shell-part="bottom-dock"]').evaluate((element)=>{const s=getComputedStyle(element);return{opacity:Number(s.opacity),pointerEvents:s.pointerEvents};});
...
async function verifyReducedMotion(browser) {
  ...
  const page=await context.newPage();
  await page.goto(baseUrl,{waitUntil:'networkidle',timeout:30000});
  await assertUi9OrLater(page,'reduced-motion');
...
try {
  ...
  const result={passed:true,profiles:profiles.map((p)=>p.name),runtimeAtLeastUi9:true,rotation:true,reducedMotion:true,exitPresence:true,touch44:true,back:true};
