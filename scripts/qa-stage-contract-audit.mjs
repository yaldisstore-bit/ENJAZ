...
for (const token of [
  ...
  'audit:ui-v2:freeze', 'audit:phase4-2:daily-work', 'test:functional', 'audit:secrets',
...
if (failures.length) {
  console.error('ENJAZ QA stage contract FAILED. Canonical UI V2 certification was weakened.');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('ENJAZ QA stage contract passed: canonical main retains UI V2 freeze, Phase 4.2, functional, build, browser and deployed-external gates.');
