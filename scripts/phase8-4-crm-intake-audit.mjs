...
for(const m of ['Status: **IN PROGRESS — NOT CLOSABLE YET**','HTTP 409 with `STORAGE_OBJECT_NOT_FOUND`','HTTP 200 with `uploadStatus=acknowledged`','bucket is `public=false`','single 14-byte probe object still exists','known closure blocker','Phase 8.5 must remain locked'])must(evidence,m,'Real Cloud evidence');
if(state.realCloudAuthenticatedProbe!=='PASS'||state.realStorageAcknowledgementProbe!=='PASS_WITH_CLEANUP_BLOCKER'||state.realCloudEvidence!=='docs/PHASE8_4_REAL_CLOUD_EVIDENCE.md'||state.exitGatePassed!==false||!Array.isArray(state.knownBlockers)||state.knownBlockers.length!==1)throw new Error('Real Cloud evidence/blocker state drift');
