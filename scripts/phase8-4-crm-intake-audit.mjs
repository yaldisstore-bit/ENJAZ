...
const kickoff=read('docs/PHASE8_4_KICKOFF.md'),state=JSON.parse(read('docs/PHASE8_4_STATE.json')),evidence=read('docs/PHASE8_4_REAL_CLOUD_EVIDENCE.md'),sql=read('database/migrations/phase_8_4_crm_smart_intake_m6_m17.sql'),hardening=read('database/migrations/phase_8_4_rpc_security_hardening.sql'),edge=read('supabase/functions/enjaz-intake-upload/index.ts'),service=read('src/features/crm-intake/crmIntakeCommands.ts'),tests=read('tests/crmIntakeEngine.test.ts'),internalUi=read('src/ui-r2/crm-intake/LiveCrmIntakeExperience.tsx'),publicUi=read('src/ui-r2/crm-intake/PublicIntakeExperience.tsx'),browser=read('tests-external/phase8-4-crm-intake.spec.cjs');
...
for(const m of ['Status: **IN PROGRESS — CLOUD CLEAN; PR GATES PENDING**','HTTP 409 with `STORAGE_OBJECT_NOT_FOUND`','HTTP 200 with `uploadStatus=acknowledged`','bucket is `public=false`','Storage objects: **0**','related audit events: **0**','There is no remaining Real Cloud or Storage cleanup blocker','exact PR head passes the complete required GitHub gate matrix'])must(evidence,m,'Real Cloud evidence');
if(state.realCloudAuthenticatedProbe!=='PASS'||state.realStorageAcknowledgementProbe!=='PASS_ZERO_RESIDUE'||state.realCloudEvidence!=='docs/PHASE8_4_REAL_CLOUD_EVIDENCE.md'||state.exitGatePassed!==false||!Array.isArray(state.knownBlockers)||state.knownBlockers.length!==0)throw new Error('Real Cloud evidence/zero-residue state drift');
