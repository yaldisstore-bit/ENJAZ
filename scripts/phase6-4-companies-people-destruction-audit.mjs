...
for (const marker of ['relationBoundary','if (from === null || to === null) return false','return from <= now && to > now']) if (!contactService.includes(marker)) errors.push(`malformed relationship fail-closed fix missing: ${marker}`);
