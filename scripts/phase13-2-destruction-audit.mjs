...
req(s.phase==='13.2'&&['IN_PROGRESS','CLOSED'].includes(s.status)&&['A3_DESTRUCTION_AND_CLOSURE_READINESS','PR_CERTIFICATION_READY'].includes(s.currentSlice),'13.2 A3 lifecycle invalid');
req(s.a1Status==='CERTIFIED'&&s.a2Status==='CERTIFIED'&&['IN_PROGRESS','CERTIFIED'].includes(s.a3Status),'13.2 slice certification state invalid');
if(s.currentSlice==='PR_CERTIFICATION_READY')req(s.a3Status==='CERTIFIED'&&s.implementationPrReady===true&&s.a3GateRunId===35399744392&&s.a3GateRunNumber===9&&s.a3GateHead==='90530ce686c05b9008184254ad51ab4aecf9f336'&&s.a3PassCount===14&&s.a3FailCount===0,'13.2 PR-ready A3 certificate invalid');
...
if(s.status==='CLOSED')req(s.successorStatus==='AUTHORIZED_NEXT'&&s.phase13_3Allowed===true&&s.exitGatePassed===true&&s.closureDecision==='PASS'&&s.pullRequestGate==='PASS'&&s.postMergeRecertification==='PASS','closed 13.2 lifecycle invalid');else req(s.successorStatus==='LOCKED'&&s.phase13_3Allowed===false&&s.exitGatePassed===false&&s.closureDecision==='PENDING','A3 must keep Phase 13.3 locked');
