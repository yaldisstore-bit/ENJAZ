...
assert(files.home.includes('data-core-screen="home"'), 'canonical Home core screen missing');
assert(files.app.includes('ConnectedHomeScreen') && files.app.includes('FixtureHomeScreen'), 'canonical Home live/preview mounting contract missing');
assert(!files.screens.includes('HomeCoreScreen'), 'obsolete static Home implementation returned to CoreScreens');
for (const screen of ['operations','command','finance']) {
...
for (const token of ['فتح مركز القيادة','العودة للعمليات']) {
  assert(files.screens.includes(token), `core interaction missing: ${token}`);
}
assert(files.home.includes('فتح المعاملة'), 'Home priority interaction missing: فتح المعاملة');
...
for (const forbidden of ['واجهة بحث تجريبية','نتائج تجريبية','UI-6','Reality Gate','PROOF','AUDIT']) {
  assert(!files.app.includes(forbidden) && !files.screens.includes(forbidden) && !files.home.includes(forbidden) && !files.daily.includes(forbidden) && !files.shell.includes(forbidden), `developer terminology leaked: ${forbidden}`);
}

assert(files.main.includes("./ui-v2/styles/core.css"), 'core stylesheet is not loaded');
assert(files.main.includes("./ui-v2/styles/home-dashboard.css"), 'Home stylesheet is not loaded');
...
assert(files.homeCss.includes('@media (max-width: 360px)'), 'Home narrow-phone rules missing');
assert(files.dailyCss.includes('@media (max-width: 390px)'), 'Daily Work narrow-phone rules missing');
assert(!files.app.includes('ui-rebirth') && !files.screens.includes('ui-rebirth') && !files.home.includes('ui-rebirth') && !files.daily.includes('ui-rebirth'), 'core runtime depends on quarantined legacy UI');

console.log('UI-6 core audit PASS');
console.log('- CoreApp remains the product core behind production/fixture runtime wrappers');
console.log('- canonical Home plus Daily Work/Operations/Command/Finance core destinations exist');
console.log('- obsolete static Home cannot satisfy the gate');
