...
- **A3-D — `document.request`: IN_PROGRESS** — adds only create-only client document requests through existing M3 `get_client_portal_admin_authority_v1` + `save_client_portal_request_v1`; request type is fixed to document, updates/resource shares/other portal request types remain forbidden.
...
- **Phase 12.3 — Agentic ENJAZ Copilot — M9: IN_PROGRESS; A1 + A2 + A3-A + A3-B + A3-C PASS; A3-D `document.request` is the current slice.**
