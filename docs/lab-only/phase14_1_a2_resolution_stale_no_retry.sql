-- LAB ONLY: nqhgaukutkyvfumbtbtg. No production deployment.
-- Replace only the existing resolution function; no fixture rows are changed.
DO $enjaz_guard$
BEGIN
  IF (SELECT count(*) FROM storage.objects) <> 0 OR
     NOT (
