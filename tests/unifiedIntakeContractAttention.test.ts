...
import {createUnifiedAttentionGateway} from '../src/features/intake-contract-communication/unifiedAttention.ts';
...
test('unified attention calls projection RPC and parses staff-safe items',async()=>{
 ...
 const out=await createUnifiedAttentionGateway(client).list({workspaceId:W});
...
test('filter and terminal options are carried to governed read RPC',async()=>{
 ...
 await createUnifiedAttentionGateway(client).list({workspaceId:W,kind:'contract_renewal',includeTerminal:true,limit:50});
...
test('invalid ids and limits fail before network',async()=>{
 let calls=0;const gateway=createUnifiedAttentionGateway({rpc:()=>{calls++;return Promise.resolve({data:payload,error:null})}} as any);
 await assert.rejects(()=>gateway.list({workspaceId:'bad'}),e=>e instanceof DataAccessError&&e.dataCode==='DATA_VALIDATION_FAILED');
 await assert.rejects(()=>gateway.list({workspaceId:W,limit:501}),e=>e instanceof DataAccessError&&e.dataCode==='DATA_VALIDATION_FAILED');
...
test('malformed projection fails closed',async()=>{
 ...
 await assert.rejects(()=>createUnifiedAttentionGateway(client).list({workspaceId:W}),e=>e instanceof DataAccessError);
});
