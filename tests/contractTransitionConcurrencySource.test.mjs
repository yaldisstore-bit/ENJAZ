...
function violations(s=sql,g=gateway,p=panel){
  ...
  req(s.includes('p_operation_id uuid')&&s.includes('p_expected_version integer'),'v2-operation-version-input');
...
test('destruction: removing operation id is detected',()=>assert.ok(violations(sql.replace('p_operation_id uuid','uuid')).includes('v2-operation-version-input')));
test('destruction: removing expected version is detected',()=>assert.ok(violations(sql.replace('p_expected_version integer','integer')).includes('v2-operation-version-input')));
...
test('destruction: legacy v1 browser grant is detected',()=>assert.ok(violations(sql.replace('from public,anon,authenticated,service_role','from public,anon,service_role')).includes('legacy-v1-browser-revoked')));
