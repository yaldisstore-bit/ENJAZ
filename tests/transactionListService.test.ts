...
import { loadTransactionListSource, TransactionListCapacityError, TransactionWorkspaceUnavailableError } from '../src/features/transactions/transactionListService.ts';
...
function transaction(id: string): RowOf<'transactions'> {
  return {
    id, workspace_id: WORKSPACE_ID, company_id: COMPANY_ID, primary_contact_id: null, type: 'معاملة اختبار', department: null,
    ...
    deletion_reason: null, legacy_id: null, legacy_source: null,
  };
}
...
test('transaction list source paginates authoritative transactions and loads referenced companies', async () => {
  ...
  const factory = {
    ...
    forWorkspace(id: string) { assert.equal(id, WORKSPACE_ID); return layer; },
  } as EnjazDataLayerFactory;
...
test('transaction source fails closed above the 5000 row safety ceiling', async () => {
  const rows = Array.from({ length: 5_001 }, (_, index) => transaction(`capacity-${String(index).padStart(5, '0')}`));
  ...
  const layer = {
    ...
    companies: { async list() { companyRead = true; throw new Error('companies must not load after capacity failure'); } },
  } as unknown as EnjazWorkspaceDataLayer;
  const factory = { async resolveWorkspaceId() { return WORKSPACE_ID; }, forWorkspace() { return layer; } } as EnjazDataLayerFactory;
  await assert.rejects(() => loadTransactionListSource(factory, '44444444-4444-4444-8444-444444444444'), TransactionListCapacityError);
