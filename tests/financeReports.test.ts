...
test('company report cannot absorb unrelated company, unscoped ledger, or unattributable cashbox opening balance', () => {
  const report = buildFinancialReport(source({
  ...
  assert.equal(report.totals.openingBalanceCents, 0n);
  assert.ok(report.movements.every((item) => item.companyId === C1 || item.transactionId === T1));
});

test('transaction report has direct drill-down provenance, deterministic ordering, and no workspace cashbox balance leakage', () => {
  const report = buildFinancialReport(source({
...
test('irrelevant target identifiers cannot contaminate another report kind', () => {
  const report = buildFinancialReport(source(), { kind: 'period', companyId: 'missing', transactionId: 'missing', cashboxId: 'missing' }, GENERATED);
  assert.equal(report.scope.companyId, null);
  assert.equal(report.scope.transactionId, null);
  assert.equal(report.scope.cashboxId, null);
});
