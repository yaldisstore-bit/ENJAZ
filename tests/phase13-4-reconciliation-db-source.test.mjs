...
test('A2 denies anonymous/cross-workspace/foreign import jobs and binds the durable original payload hash', () => {
  for (const marker of [
    ...
    "j.reconciliation->'result'->>'schema' =",
    "'enjaz.legacy.ordered-import.execution-result.v1'",
    "j.reconciliation->'result'->>'atomic' = 'true'",
    "j.reconciliation->'result'->>'persistenceperformed' = 'true'",
    "j.reconciliation->'result'->'counts' = (j.counts - 'contract')",
    "j.reconciliation->'result'->>'payloadhash' =",
