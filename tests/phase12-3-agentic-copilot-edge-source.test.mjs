...
function violations(source=edge){
  ...
  req(source.includes("admin.rpc('copilot_begin_request_v6'"),'service-trace-boundary-v6');
  ...
  req(!/admin\.rpc\(['"](?:copilot_execute_(?:followup_(?:snooze|create)|schedule_reminder)_v1|post_payment|reverse_payment|mutate_transaction|create_|update_|delete_|archive_|send_|schedule_)/.test(source),'no-service-business-mutation-rpc');
  req(!/userClient\.rpc\(['"](?!global_search_v1|get_scheduling_deadline_snapshot_v1|copilot_execute_followup_snooze_v1|copilot_execute_followup_create_v1|copilot_execute_schedule_reminder_v1)[^'"]*(?:post_payment|reverse_payment|mutate_transaction|create_|update_|delete_|archive_|send_|schedule_)/.test(source),'only-three-allowlisted-user-mutation-rpcs');
...
test('12.3 A3-C Edge has exactly three action-specific mutation paths',()=>assert.deepEqual(violations(),[]));
...
test('destruction: fourth user mutation RPC detected',()=>assert.ok(violations(edge+"\nuserClient.rpc('post_payment_v1',{});").includes('only-three-allowlisted-user-mutation-rpcs')));
