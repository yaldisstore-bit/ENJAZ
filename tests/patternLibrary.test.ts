...
import { PATTERN_DENSITIES, PATTERN_FAMILIES, PATTERN_GUARDS, RISK_LEVELS, SYSTEM_STATE_TONES, clampPercent, formatIqd } from '../src/design-system/patterns/patternContract.ts';
...
test('premium pattern family contract is complete and unique', () => {
  ...
  for (const family of ['transaction', 'company', 'contact', 'finance', 'risk', 'timeline', 'followUp', 'workflow', 'automation', 'command', 'search', 'actionMenu', 'systemState', 'skeleton']) assert.ok(PATTERN_FAMILIES.includes(family as (typeof PATTERN_FAMILIES)[number]));
});
...
test('pattern density remains intentionally bounded', () => assert.deepEqual([...PATTERN_DENSITIES], ['comfortable', 'compact']));
...
test('risk and system state vocabularies cover destructive product states', () => {
  ...
  for (const tone of ['empty', 'loading', 'success', 'warning', 'error', 'conflict', 'offline', 'recovery']) assert.ok(SYSTEM_STATE_TONES.includes(tone as (typeof SYSTEM_STATE_TONES)[number]));
});
...
test('pattern index exposes every composite family implementation', () => {
  ...
  for (const file of ['EntityPatterns.tsx', 'OperationsPatterns.tsx', 'DiscoveryPatterns.tsx', 'patternContract.ts']) assert.ok(index.includes(file));
});

test('pattern CSS is split, token-only and contains compact mobile contracts', () => {
  const aggregate = read('src/styles/patterns.css');
  for (const sheet of ['patterns-entities.css', 'patterns-operations.css', 'patterns-discovery.css', 'patterns-responsive.css']) assert.ok(aggregate.includes(sheet));
  const css = [aggregate, read('src/styles/patterns-entities.css'), read('src/styles/patterns-operations.css'), read('src/styles/patterns-discovery.css'), read('src/styles/patterns-responsive.css'), read('src/styles/pattern-lab.css')].join('\n');
...
test('Pattern Lab proves every required domain and recovery state', () => {
  ...
  for (const marker of ['TransactionPattern', 'CompanyPattern', 'ContactPattern', 'FinanceSummaryPattern', 'RiskSignalPattern', 'TimelinePattern', 'FollowUpPattern', 'WorkflowPattern', 'AutomationPattern', 'CommandModulePattern', 'SearchResultPattern', 'ActionMenuPattern', 'PatternSkeleton', 'tone="empty"', 'tone="loading"', 'tone="success"', 'tone="warning"', 'tone="error"', 'tone="conflict"', 'tone="offline"', 'tone="recovery"', 'density="compact"']) assert.ok(lab.includes(marker), `missing Pattern Lab marker: ${marker}`);
});
